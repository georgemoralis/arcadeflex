...
public class tilemapC {
    ...
    /*TODO*///
    static char[] flip_bit_table = new char[0x100];
    /* horizontal flip for 8 pixels */
    static tilemap first_tilemap;
    /* resource tracking */
    ...
    public static int MASKROWBYTES(int W) {
        return (W + 7) / 8;
    }
    ...
    static void memcpybitmask8(UBytePtr dest, UBytePtr source, UBytePtr bitmask, int count) {
        ...
        for (;;) {
            if (dest.offset < 0) {
                break;//hacked for system16 games not to crash
            }
            if (source.offset < 0) {
                break;
            }
            int data = bitmask.read(bi++) & 0xFF;
            if ((dest.offset + di + 0) >= dest.memory.length) {
                break; //hacked for system16 games not to crash
            }
            if ((data & 0x80) != 0) {
                dest.write(di + 0, source.read(si + 0));
            }
            if ((dest.offset + di + 1) >= dest.memory.length) {
                break; //hacked for system16 games not to crash
            }
            if ((data & 0x40) != 0) {
                dest.write(di + 1, source.read(si + 1));
            }
            if ((dest.offset + di + 2) >= dest.memory.length) {
                break; //hacked for system16 games not to crash
            }
            if ((data & 0x20) != 0) {
                dest.write(di + 2, source.read(si + 2));
            }
            if ((dest.offset + di + 3) >= dest.memory.length) {
                break; //hacked for system16 games not to crash
            }
            if ((data & 0x10) != 0) {
                dest.write(di + 3, source.read(si + 3));
            }
            if ((dest.offset + di + 4) >= dest.memory.length) {
                break; //hacked for system16 games not to crash
            }
            if ((data & 0x08) != 0) {
                dest.write(di + 4, source.read(si + 4));
            }
            if ((dest.offset + di + 5) >= dest.memory.length) {
                break; //hacked for system16 games not to crash
            }
            if ((data & 0x04) != 0) {
                dest.write(di + 5, source.read(si + 5));
            }
            if ((dest.offset + di + 6) >= dest.memory.length) {
                break; //hacked for system16 games not to crash
            }
            if ((data & 0x02) != 0) {
                dest.write(di + 6, source.read(si + 6));
            }
            if ((dest.offset + di + 7) >= dest.memory.length) {
                break; //hacked for system16 games not to crash
            }
            if ((data & 0x01) != 0) {
                dest.write(di + 7, source.read(si + 7));
            }
            if (--count == 0) {
                break;
            }
    ...
    public static void tilemap_set_clip(tilemap _tilemap, rectangle clip) {
        ...
        if (tilemapslog != null) {
            fprintf(tilemapslog, "clip: %d,%d,%d,%d\n", left, top, right, bottom);
        }
    }
    ...
    public static tilemap tilemap_create(WriteHandlerPtr tile_get_info,
        ...
        if (_tilemap != null) {
            ...
            if (_tilemap.mark_visible != null && _tilemap.draw != null) {
                ...
                _tilemap.transparent_pen = -1;
                /* default (this is supplied by video driver) */
    ...
    public static void tilemap_mark_tile_dirty(tilemap _tilemap, int col, int row) {
        ...
        if ((_tilemap.orientation & ORIENTATION_FLIP_X) != 0) {
            col = _tilemap.num_cols - 1 - col;
        }
        if ((_tilemap.orientation & ORIENTATION_FLIP_Y) != 0) {
            row = _tilemap.num_rows - 1 - row;
        }
    ...
    public static void tilemap_mark_all_pixels_dirty(tilemap _tilemap) {
        if (_tilemap == ALL_TILEMAPS) {
            ...
            int tile_index;
            int num_pens = _tilemap.tile_width * _tilemap.tile_height;
            /* precalc - needed for >4bpp pen management handling */
            for (tile_index = 0; tile_index < _tilemap.num_tiles; tile_index++) {
                if (_tilemap.visible[tile_index] == 0) {
                    ...
                    if (the_color != null) {
                        /*unsigned */
                        int old_pen_usage = _tilemap.pen_usage[tile_index];
                        if (old_pen_usage != 0) {
                    ...
                    if (dirty_pixels[tile_index] != 0 && visible[tile_index] != 0) {
                        ...
                        if ((type & TILEMAP_BITMASK) != 0) {
                            ...
                            if ((flags & TILE_IGNORE_TRANSPARENCY) != 0) {
                                ...
                                if (tilemapslog != null) {
                                    fprintf(tilemapslog, "pen_mask = %d\n", pen_mask);
                                }
                                if (tilemapslog != null) {
                                    fprintf(tilemapslog, "fg_transmask = %d\n", fg_transmask);
                                }
                                if (tilemapslog != null) {
                                    fprintf(tilemapslog, "bg_transmask = %d\n", bg_transmask);
                                }
...
//                                            throw new UnsupportedOperationException("tilemap rendered unimplemented");
    ...
    public static void tilemap_update(tilemap _tilemap) {
        if (_tilemap == ALL_TILEMAPS) {
                ...
                if (rows == 0 && cols == 0) {
                    ...
                    while (col < cols) {
                        ...
                        if (scroll != TILE_LINE_DISABLED) {
                            ...
                            if (scroll < 0) {
                                scroll = blit.source_height - (-scroll) % blit.source_height;
                            } else {
                                scroll %= blit.source_height;
                            }
                    ...
                    while (row < rows) {
                        ...
                        if (scroll != TILE_LINE_DISABLED) {
                            ...
                            if (scroll < 0) {
                                scroll = blit.source_width - (-scroll) % blit.source_width;
                            } else {
                                scroll %= blit.source_width;
                            }
                    ...
                    if (rowscroll[0] < 0) {
                        scrollx = blit.source_width - (-rowscroll[0]) % blit.source_width;
                    } else {
                        scrollx = rowscroll[0] % blit.source_width;
                    }

                    if (colscroll[0] < 0) {
                        scrolly = blit.source_height - (-colscroll[0]) % blit.source_height;
                    } else {
                        scrolly = colscroll[0] % blit.source_height;
                    }
...

            {
                ...
                int tile_flip = 0;
                if ((_tilemap.attributes & TILEMAP_FLIPX) != 0) {
                    tile_flip |= TILE_FLIPX;
                }
                if ((_tilemap.attributes & TILEMAP_FLIPY) != 0) {
                    tile_flip |= TILE_FLIPY;
                }
    ...
    public static void tilemap_set_scrollx(tilemap _tilemap, int which, int value) {
        ...
        if ((_tilemap.orientation & ORIENTATION_SWAP_XY) != 0) {
            if ((_tilemap.orientation & ORIENTATION_FLIP_X) != 0) {
                which = _tilemap.scroll_cols - 1 - which;
            }
            if ((_tilemap.orientation & ORIENTATION_FLIP_Y) != 0) {
                value = screen_height - _tilemap.height - value;
            }
            ...
            if ((_tilemap.orientation & ORIENTATION_FLIP_Y) != 0) {
                which = _tilemap.scroll_rows - 1 - which;
            }
            if ((_tilemap.orientation & ORIENTATION_FLIP_X) != 0) {
                value = screen_width - _tilemap.width - value;
            }
...
//                                                    throw new UnsupportedOperationException("tilemap draw8x8x8BPP unimplemented");
    ...
    public static WriteHandlerPtr draw_opaque8x8x8BPP = new WriteHandlerPtr() {
        public void handler(int xpos, int ypos) {
            ...
            if (x1 < x2 && y1 < y2) {
                /* do nothing if totally clipped */
 /*UINT8*/
                char priority = (char) (blit.priority & 0xFF);
 ...
 /*UINT8*/
    ...
    public static WriteHandlerPtr mark_visible8x8x8BPP = new WriteHandlerPtr() {
        public void handler(int xpos, int ypos) {
            int x1 = xpos;
            int y1 = ypos;
            int x2 = xpos + blit.source_width;
    ...
    public static WriteHandlerPtr mark_visible16x16x8BPP = new WriteHandlerPtr() {
        public void handler(int xpos, int ypos) {
            int x1 = xpos;
            int y1 = ypos;
            int x2 = xpos + blit.source_width;
