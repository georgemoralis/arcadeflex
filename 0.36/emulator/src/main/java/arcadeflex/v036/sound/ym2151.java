...
public class ym2151 {
 ...
 /*note: each operator number 0 contains channel specific data*/
        ...
        int FB;
        /*operator self feedback value used only by operators 0*/
        int FB0;
        /*previous output value*/
        int KC;
        /*operator KC (copied to all operators)*/
        int KCindex;
        /*speedup*/
        int KF;
        /*operator KF (copied to all operators)*/
        int PMS;
        /*channel PMS*/
        int AMS;
        /*channel AMS*/
 /*End of channel specific data*/
        ...
        int delta_AR;
        /*volume delta for attack phase*/
        int TL;
        /*Total attenuation Level*/
        int volume;
        /*operator attenuation level*/
        int delta_D1R;
        /*volume delta for decay phase*/
        int D1L;
        /*EG switches to D2R, when envelope reaches this level*/
        int delta_D2R;
        /*volume delta for sustain phase*/
        int delta_RR;
        /*volume delta for release phase*/
        ...
        int a_vol;
        /*used for attack phase calculations*/
    ...
    public static class _YM2151 {
        ...
        OscilRec[] Oscils;
        /*there are 32 operators in YM2151*/

        int[] PAN;
        /*channels output masks (0xffffffff = enable)*/

        int LFOphase;
        /*accumulated LFO phase         */
        int LFOfrq;
        /*LFO frequency                 */
        int LFOwave;
        ...
        int CT;
        /*output control pins (bit7 CT2, bit6 CT1)*/
        int noise;
        /*noise register (bit 7 - noise enable, bits 4-0 - noise freq*/

        int IRQenable;
        /*IRQ enable for timer B (bit 3) and timer A (bit 2)*/
 /*unsigned*/ int status;
        /*chip status (BUSY, IRQ Flags)*/


        Object TimATimer, TimBTimer;
        /*ASG 980324 -- added for tracking timers*/
        double[] TimerATime;
        /*Timer A times for MAME*/
        double[] TimerBTime;
        /*Timer B times for MAME*/
 ...
 /*
    	...
    	*   after it has been taken from frequency-deltas table.
         */
        int[] DT1freq;
        /*8 DT1 levels,16 MUL lelvels, 32 KC values*/

        int[] EG_tab;
        /*envelope deltas (32 + 64 rates + 32 RKS)*/

        int[] LFOfreq;
        /*frequency deltas for LFO*/

        WriteYmHandlerPtr irqhandler;
        /*IRQ function handler*/
        WriteHandlerPtr porthandler;
        /*port write function handler*/

 /*unsigned*/ int clock;
        /*chip clock in Hz (passed from 2151intf.c)*/
 /*unsigned*/ int sampfreq;
    ...
    **  Shifts below are subject to change when sampling frequency changes...
     */
    public static final int FREQ_SH = 16;
    /* 16.16 fixed point (frequency calculations) */
    public static final int ENV_SH = 16;
    /* 16.16 fixed point (envelope calculations)  */
    public static final int LFO_SH = 23;
    /*  9.23 fixed point (LFO calculations)       */
    public static final int TIMER_SH = 16;
    ...
    public static final int MAX_ATT_INDEX = ((ENV_LEN << ENV_SH) - 1);
    /*1023.ffff*/
    public static final int MIN_ATT_INDEX = ((1 << ENV_SH) - 1);
    /*   0.ffff*/
    ...
    public static final int TL_RES_LEN = (256);
    ...
    static _YM2151[] YMPSG = null;
    /* array of YM2151's */
    static int YMNumChips;
    /* total # of YM2151's emulated */
            ...
            for (i = 0; i < LFO_LEN; i++) {
                switch (x) {
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        /* triangle (255 down to 0, up to 255) */
                        if (i < 256) {
                            m = 255 - i;
                        } else {
                            m = i - 256;
 ...
 /* we reach m = zero here !!!*/
            ...
            for (i = 0; i < LFO_LEN; i++) {
                switch (x) {
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        /* triangle (0 to 127,127 to -128,-127 to 0) */
                        if (i < 128) {
                            m = i; /*0 to 127*/
                        } else {
                            if (i < 384) {
        ...
        for (i = 0; i < 16; i++) {
            ...
            /*every 3 'dB' except for all bits = 1 = 45dB+48dB*/
    ...
    static void init_chip_tables(_YM2151 chip) {
        ...
        double scaler;
        /* formula below is true for chip clock=3579545 */
 ...
 /*calculate phase increment*/
        ...
        for (i = 0; i < 256; i++) {
            ...
            /*calculate phase increment*/
            chip.LFOfreq[0xff - i] = (int) (((pom * LFO_LEN) / (double) chip.sampfreq) * mult);
            /*fixed point*/
 ...
 /* User's Manual pages 15,16  */
    ...
    public static TimerCallbackHandlerPtr TimerCallbackHandlerPtr_a = new TimerCallbackHandlerPtr() {
        public void handler(int n) {
            ...
            if ((chip.IRQenable & 0x04) != 0) {
                ...
                chip.status |= 1;
                if ((oldstate == 0) && (chip.irqhandler != null)) {
                    (chip.irqhandler).handler(1);
                }
            }
        }
    };
    public static TimerCallbackHandlerPtr TimerCallbackHandlerPtr_b = new TimerCallbackHandlerPtr() {
        public void handler(int n) {
            _YM2151 chip = YMPSG[n];
 ...
 /*if ((v&0x80)) printf("\nYM2151 CSM MODE ON\n");*/
                        ...
                        chip.IRQenable = v;
                        /*bit 3-timer B, bit 2-timer A*/

                        if ((v & 0x20) != 0) /*reset timer B irq flag*/ {
                            int oldstate = chip.status & 3;
                            chip.status &= 0xfd;
                            if ((oldstate == 2) && (chip.irqhandler) != null) {
                                (chip.irqhandler).handler(0);
                            }
                        }
                        ...
                        if ((v & 0x10) != 0) /*reset timer A irq flag*/ {
                            int oldstate = chip.status & 3;
                            chip.status &= 0xfe;
                            if ((oldstate == 1) && (chip.irqhandler) != null) {
                                (chip.irqhandler).handler(0);
                            }
 ...
 /* ASG 980324: added a real timer */
                    ...
                    case 0x1b:
                        ...
                        if (chip.porthandler != null) {
                            (chip.porthandler).handler(0, (int) ((chip.CT) >> 6));
                        }
                        break;

                    default:
                        if (errorlog != null) {
                            fprintf(errorlog, "YM2151 Write %02x to undocumented register #%02x\n", v, r);
                        }
                        break;
                }
                break;
    ...
    ** 'rate' is sampling rate
    ...
    public static int YM2151Init(int num, int clock, int rate) {
        ...
        for (i = 0; i < YMNumChips; i++) {
            ...
            YMPSG[i].clock = clock;
            /*rate = clock/64;*/
            YMPSG[i].sampfreq = rate != 0 ? rate : 44100;
            /* avoid division by 0 in init_chip_tables() */
    ...
    ** reset all chip registers.
    ...
    public static void YM2151ResetChip(int num) {
        ...
        /*only because of CT1, CT2 output pins*/
    ...
    public static void chan_calc(int chan) {
        ...
        env = unsigned(volume_calc(OP[op_offset], AM));
        {
            ...
            if (PSG.Oscils[op_offset].connect == null) /* algorithm 5 */ {
        ...
        do {
            ...
            switch (op[op_offset].state) {
                case EG_ATT: /*attack phase*/ {
                    ...
                    step = (step >> ENV_SH) - (unsigned(op[op_offset].volume) >> ENV_SH);
                    /*number of levels passed since last time*/
                    if (step > 0) {
                        int tmp_volume;

                        tmp_volume = op[op_offset].volume + (step << ENV_SH);
                ...
                case EG_DEC:
                    ...
                    if ((op[op_offset].volume += op[op_offset].delta_D1R) >= op[op_offset].D1L) {
                        op[op_offset].volume = (int) (op[op_offset].D1L);
                        /*this is not quite correct (checked)*/
