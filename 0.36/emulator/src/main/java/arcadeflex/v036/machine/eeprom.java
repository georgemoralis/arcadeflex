...
public class eeprom {
    ...
    public static void EEPROM_write(int bit) {
        ...
        if (eepromlog != null) {
            fprintf(eepromlog, "EEPROM write bit %d\n", bit);
        }

        if (serial_count >= SERIAL_BUFFER_LENGTH - 1) {
            if (eepromlog != null) {
                fprintf(eepromlog, "error: EEPROM serial buffer overflow\n");
            }
            return;
        }
        ...
        /* nul terminate so we can treat it as a string */

        if (eepromlog != null) {
            fprintf(eepromlog, "EEPROM write bit, buffer = " + new String(serial_buffer) + "\n");
        }
        if (eepromlog != null) {
            fprintf(eepromlog, "EEPROM write bit, serial_count = %d\n", serial_count);
        }
        if ((intf.cmd_read != null)
            ...
            serial_count = 0;
            if (eepromlog != null) {
                fprintf(eepromlog, "EEPROM read %04x from address %02x\n", eeprom_data_bits, address);
            }
            ...
            if (eepromlog != null) {
                fprintf(eepromlog, "EEPROM erase address %02x\n", address);
            }
            if (locked == 0) {
                if (intf.data_bits == 16) {
                ...
                if (eepromlog != null) {
                    fprintf(eepromlog, "Error: EEPROM is locked\n");
                }
                serial_count = 0;
            }
            ...
            if (eepromlog != null) {
                fprintf(eepromlog, "EEPROM write %04x to address %02x\n", data, address);
            }
            if (locked == 0) {
                if (intf.data_bits == 16) {
            ...
            if (eepromlog != null) {
                fprintf(eepromlog, "EEPROM unlock\n");
            }
            locked = 0;
            serial_count = 0;
        }
    ...
    public static void EEPROM_set_cs_line(int state) {
        //#if VERBOSE
        if (eepromlog != null) {
            fprintf(eepromlog, "set reset line %d\n", state);
        }
        //#endif
        reset_line = state;
    ...
    public static void EEPROM_set_clock_line(int state) {
        //#if VERBOSE
        if (eepromlog != null) {
            fprintf(eepromlog, "set clock line %d\n", state);
        }
        //#endif
