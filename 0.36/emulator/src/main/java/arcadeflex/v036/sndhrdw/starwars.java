...
public class starwars {
 ...
 /* d0 (out) TMS5220 Not Write       */
    static int port_B = 0;
    /* 6532 port B data register        */
 /* (interfaces to TMS5220 data bus) */

    static int irq_flag = 0;
    /* 6532 interrupt flag register */

    static int port_A_ddr = 0;
    /* 6532 Data Direction Register A */
    static int port_B_ddr = 0;
    /* 6532 Data Direction Register B */
 /* for each bit, 0 = input, 1 = output */

    static int PA7_irq = 0;
    /* IRQ-on-write flag (sound CPU) */

    /**
     * *****************************************************
     */
    static TimerCallbackHandlerPtr snd_interrupt = new TimerCallbackHandlerPtr() {
        @Override
    ...
    public static ReadHandlerPtr starwars_m6532_r = new ReadHandlerPtr() {
 ...
 /* Note: bit 4 is always set to avoid sound self test */
    ...
    public static WriteHandlerPtr starwars_m6532_w = new WriteHandlerPtr() {
 ...
 /* sound CPU, depending on the state of this flag.   */
                ...
                case 0x1f:
                    /* 0x9f - Set Timer to decrement every n*1024 clocks, */
 ...
 /* 6532 runs at 1.5 MHz, so there a 3 cylces in 2 usec */
    ...
    /* data for the sound cpu */
    static int main_data;
    /* data for the main  cpu */
 ...
 /* communicate with the Main CPU                        */
     ...
     * *****************************************************
     */
    public static ReadHandlerPtr starwars_sin_r = new ReadHandlerPtr() {
        public int handler(int offset) {
            int res;

            port_A &= 0x7f;
            /* ready to receive new commands from main */
            res = sound_data;
            sound_data = 0;
            return res;
        }
    ...
    public static WriteHandlerPtr starwars_sout_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            port_A |= 0x40;
            /* result from sound cpu pending */
            main_data = data;
            return;
        }
    };
 ...
 /* CPU communications                                   */
     ...
     * *****************************************************
     */
    public static ReadHandlerPtr starwars_main_read_r = new ReadHandlerPtr() {
        public int handler(int offset) {
            int res;

            //logerror("main_read_r\n");
            port_A &= 0xbf;
            /* ready to receive new commands from sound cpu */
            res = main_data;
            main_data = 0;
            return res;
        }
     ...
     * *****************************************************
     */
    public static ReadHandlerPtr starwars_main_ready_flag_r = new ReadHandlerPtr() {
        public int handler(int offset) {
            /*TODO*///	#if 0 /* correct, but doesn't work */
/*TODO*///		return (port_A & 0xc0); /* only upper two flag bits mapped */
/*TODO*///	#else
            return (port_A & 0x40);
            /* sound cpu always ready */
 /*TODO*///	#endif
        }
    };
     ...
     * *****************************************************
     */
    public static WriteHandlerPtr starwars_main_wr_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            port_A |= 0x80;
            /* command from main cpu pending */
            sound_data = data;
            if (PA7_irq != 0) {
                cpu_cause_interrupt(1, M6809_INT_IRQ);
            }
     ...
     * *****************************************************
     */
    public static WriteHandlerPtr starwars_soundrst = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            port_A &= 0x3f;
