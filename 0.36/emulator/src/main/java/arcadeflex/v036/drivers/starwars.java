 ...
 * Changelog
 * =========
 * 02/02/2023 - shadow - This file should be complete for 0.36 version
 */
...
//machine imports
import static arcadeflex.v036.machine.starwars.*;
import static arcadeflex.v036.machine.swmathbx.*;
import static arcadeflex.v036.machine.slapstic.*;
//mame imports
...
//sndhrdw imports
import static arcadeflex.v036.sndhrdw.starwars.*;
//sound imports
...
public class starwars {
    ...
    public static WriteHandlerPtr starwars_out_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            ...
            switch (offset) {
                ...
                case 2:
                    /* LED 3 */
                    osd_led_w.handler(2, data >> 7);
                    break;
                case 3:
                    /* LED 2 */
                    osd_led_w.handler(1, data >> 7);
                    break;
                case 4:
                    ...
                    break;
                case 5:
                    prngclr.handler(offset, data);
                    break;
                case 6:
                    osd_led_w.handler(0, data >> 7);
                    break;
                /* LED 1 */
                case 7:
                    if (errorlog != null) {
    ...
    public static ReadHandlerPtr esb_slapstic_tweak = new ReadHandlerPtr() {
    ...
    static opbase_handlerPtr esb_setopbase = new opbase_handlerPtr() {
		 ...
		 *		ROM, so it doesn't matter which version we're actually executing.
            ...
            if ((pc & 0xe000) == 0x8000) {
                ...
                bank = esb_slapstic_tweak.handler((pc) & 0x1fff);
                /* ASG - switched to ESB version */
 /* catching every branch during slapstic area */
                ...
                if (prevpc != 0x8080 && prevpc != 0x8090 && prevpc != 0x80a0 && prevpc != 0x80b0) {
                    bank = esb_slapstic_tweak.handler((prevpc) & 0x1fff);
                    /* ASG - switched to ESB version */
                }
            }
    ...
    public static InitMachineHandlerPtr esb_init_machine = new InitMachineHandlerPtr() {
        public void handler() {
            /* Set up the slapstic */
            slapstic_init(101);
     ...
     ************************************
     */
    public static ReadHandlerPtr esb_slapstic_r = new ReadHandlerPtr() {
        public int handler(int offset) {
            int val;

            int bank = (esb_slapstic_tweak.handler(offset) * 0x2000);
            /* ASG - switched to ESB version */
    ...
    public static WriteHandlerPtr esb_slapstic_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            //	if (errorlog) fprintf (errorlog, "esb slapstic tweak via write\n");
            esb_slapstic_tweak.handler(offset);
            /* ASG - switched to ESB version */
        }
    };
    ...
    static MemoryReadAddress readmem[]
            = {
                ...
                new MemoryReadAddress(0x4700, 0x4700, reh),
                new MemoryReadAddress(0x4701, 0x4701, rel),
                new MemoryReadAddress(0x4703, 0x4703, prng), /* pseudo random number generator */
    ...
    static MemoryWriteAddress writemem[]
            = {
                ...
                new MemoryWriteAddress(0x4600, 0x461f, avgdvg_go),
                new MemoryWriteAddress(0x4620, 0x463f, avgdvg_reset),
                ...
                new MemoryWriteAddress(0x46e0, 0x46e0, starwars_soundrst),
                new MemoryWriteAddress(0x4700, 0x4707, swmathbx),
    ...
    static InputPortHandlerPtr input_ports_starwars = new InputPortHandlerPtr() {
        public void handler() {
            PORT_START();
            /* IN0 */
    ...
    static InputPortHandlerPtr input_ports_esb = new InputPortHandlerPtr() {
        public void handler() {
            PORT_START();
            /* IN0 */
            PORT_BIT(0x01, IP_ACTIVE_LOW, IPT_COIN2);
     ...
     **************************************************************************
     */
    static RomLoadHandlerPtr rom_starwar1 = new RomLoadHandlerPtr() {
        public void handler() {
            ROM_REGION(0x12000, REGION_CPU1);
            /* 2 64k ROM spaces */
            ROM_LOAD("136021.105", 0x3000, 0x1000, 0x538e7d2f);/* 3000-3fff is 4k vector rom */
            ROM_LOAD("136021.114", 0x6000, 0x2000, 0xe75ff867);
            /* ROM 0 bank pages 0 and 1 */
 ...
 /* Sound ROMS */
            ...
            /* Really only 32k, but it looks like 64K */
    ...
    static RomLoadHandlerPtr rom_starwars = new RomLoadHandlerPtr() {
        public void handler() {
            ROM_REGION(0x12000, REGION_CPU1);
            /* 2 64k ROM spaces */
            ROM_LOAD("136021.105", 0x3000, 0x1000, 0x538e7d2f);/* 3000-3fff is 4k vector rom */
            ROM_LOAD("136021.214", 0x6000, 0x2000, 0x04f1876e);
            /* ROM 0 bank pages 0 and 1 */
            ROM_CONTINUE(0x10000, 0x2000);
            ROM_LOAD("136021.102", 0x8000, 0x2000, 0xf725e344);/*  8k ROM 1 bank */
            ROM_LOAD("136021.203", 0xa000, 0x2000, 0xf6da0a00);/*  8k ROM 2 bank */
 ...
 /* Sound ROMS */
    ...
    static RomLoadHandlerPtr rom_esb = new RomLoadHandlerPtr() {
        public void handler() {
            ROM_REGION(0x22000, REGION_CPU1);
            /* 64k for code and a buttload for the banked ROMs */
            ROM_LOAD("136031.111", 0x03000, 0x1000, 0xb1f9bd12);
            /* 3000-3fff is 4k vector rom */
