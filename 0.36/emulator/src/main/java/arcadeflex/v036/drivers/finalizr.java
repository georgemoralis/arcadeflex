...
import static arcadeflex.v036.vidhrdw.finalizr.*;
//TODO
...
public class finalizr {
    ...
    public static InterruptHandlerPtr finalizr_interrupt = new InterruptHandlerPtr() {
        public int handler() {
            if (cpu_getiloops() == 0) {
    ...
    static InputPortHandlerPtr input_ports_finalizr = new InputPortHandlerPtr() {
        public void handler() {
            PORT_START();
            /* IN2 */
    ...
    static InputPortHandlerPtr input_ports_finalizb = new InputPortHandlerPtr() {
        public void handler() {
            PORT_START();
            /* IN2 */
            PORT_BIT(0x01, IP_ACTIVE_LOW, IPT_COIN1);
     ...
     **************************************************************************
     */
    static RomLoadHandlerPtr rom_finalizr = new RomLoadHandlerPtr() {
        public void handler() {
 ...
 /* the original has a custom IC */
    ...
    static RomLoadHandlerPtr rom_finalizb = new RomLoadHandlerPtr() {
        public void handler() {
            ROM_REGION(2 * 0x10000, REGION_CPU1);/* 64k for code + 64k for decrypted opcodes */
    ...
    public static InitDriverHandlerPtr init_finalizr = new InitDriverHandlerPtr() {
        public void handler() {
            konami1_decode();
        }
    };
