    ...
    public static WriteHandlerPtr m6803_internal_registers_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            switch (offset) {
                ...
                case 0x01:
                    if (m6800.port2_ddr != data) {
                        ...
                        if ((m6800.port2_ddr & 2) != 0) {
                            if (errorlog != null) {
                                fprintf(errorlog, "CPU #%d PC %04x: warning - port 2 bit 1 set as output (OLVL) - not supported\n", cpu_getactivecpu(), cpu_get_pc());
                            }
                        }
                    }
                    break;
                ...
                case 0x07:
                    if (errorlog != null) {
                        fprintf(errorlog, "CPU #%d PC %04x: warning - write %02x to unsupported internal register %02x\n", cpu_getactivecpu(), cpu_get_pc(), data, offset);
                    }
                    break;
                case 0x08:
                    m6800.tcsr = data;
...
//if (errorlog) fprintf(errorlog,"CPU #%d PC %04x: TCSR = %02x\n",cpu_getactivecpu(),cpu_get_pc(),data);
                ...
                case 0x0e:
                    if (errorlog != null) {
                        fprintf(errorlog, "CPU #%d PC %04x: warning - write %02x to read only internal register %02x\n", cpu_getactivecpu(), cpu_get_pc(), data, offset);
                    }
                    break;
                case 0x0f:
                case 0x10:
                ...
                case 0x14:
                    if (errorlog != null) {
                        fprintf(errorlog, "CPU #%d PC %04x: write %02x to RAM control register\n", cpu_getactivecpu(), cpu_get_pc(), data);
                    }
                    m6800.ram_ctrl = data;
                    break;
                case 0x15:
                ...
                default:
                    if (errorlog != null) {
                        fprintf(errorlog, "CPU #%d PC %04x: warning - write %02x to reserved internal register %02x\n", cpu_getactivecpu(), cpu_get_pc(), data, offset);
                    }
                    break;
            }
        }
