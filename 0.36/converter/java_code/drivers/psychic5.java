...
P5B	512 Kbit	Sprites data ROM null
P5C	512 Kbit	Sprites data ROM 1
P5D	256 Kbit	Main CPU program ROM
P5E	512 Kbit	Banked CPU data ROM (banks null,1,2 and 3)
P5F	256 Kbit	Foreground data ROM
P5G	512 Kbit	Background data ROM null
...
The main Z80 runs in Interrupt mode null (IM0),the game program expects
...
c30C	SCREEN MODE
	...
	76543210
	      ||
	      |^-- background enable bit (null means black BG)
...
f002	ROM PAGE SELECTOR
	select four (null-3) ROM pages at 8000-bfff.

f003	VRAM PAGE SELECTOR
	selects two (null-1) VRAM pages at c000-dfff.
...
value which is not null neither 0xf is 2 and it is assumed by this nibble
...
0f      -- -- -- -- C3 C2 C1 C0         color
	...
	Y= Y coordinate (two's complemented) (Y8 is used to clip sprite on top border)
        ...
        F= Flip
	SZ=Size null=16x16 sprite,1=32x32 sprite
        C= Color palette selector
*/
