...
1497	print string following call: (char)*,null. program continues after null.
		base addr = c300 + HL  08ff, DE=xy , BC=dxdy
		+null<-(e61b)	+100<-D	+200<-E	+300<-char	+400<-(e61c)

1642	A<- buttons status (bits null1)

					Interesting locations (main cpu)
					--------------------------------

2907	table of routines (f150 index = 1-3f, copied to e612)
		e60e<-address of routine, f150<-null. e60e is used until f150!=null
...
7fff	country code: null <. Japan; else World

e615	rank:	null-easy	1-normal	2-hard	3-hardest
...
c8c		c000-c7ff<-null	c800-cfff<-null	f600<-f200(400)
...
port 0c <- f14c = bit null/1/2/3 = port 6/4/a/8 val < FF
...
- The bankswitching registers bits 45 are used (any Z80). What for ?
...
public class airbustr
{
	...
	*/
	...
	public static ReadHandlerPtr sharedram_r = new ReadHandlerPtr() { public int handler(int offs){ return sharedram[offs]; } };
	public static WriteHandlerPtr sharedram_w = new WriteHandlerPtr() { public void handler(int offs, int data){ sharedram[offs] = data; } };
	...
	public static WriteHandlerPtr devram_w = new WriteHandlerPtr() { public void handler(int offs, int data){	devram[offs] = data; } };
