...
public class irobot
{
	...
	public static WriteHandlerPtr irobot_statwr_w = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
		...
		if ((data & 0x04) && !(irobot_statwr & 0x04))
		{
			...
			if (irvg_running == 0)
			{
				if (errorlog != 0) fprintf(errorlog,"vg start ");
				IR_CPU_STATE;
				irvg_timer = timer_set (TIME_IN_MSEC(10), 0, irvg_done_callback);
			}
			else
			{
				if (errorlog != 0) fprintf (errorlog, "vg start [busy!] ");
				IR_CPU_STATE;
	...
	public static WriteHandlerPtr irobot_rom_banksel = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
		UINT8 *RAM = memory_region(REGION_CPU1);
	...
	static void scanline_callback(int scanline)
	{
	    ...
	    if (errorlog != 0) fprintf(errorlog,"SCANLINE CALLBACK %d\n",scanline);
