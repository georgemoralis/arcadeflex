...
public class twincobr
{
	...
	#if LOG_DSP_CALLS
		if (errorlog != 0) fprintf(errorlog,"DSP PC:%04x IO read %04x at %08x (port 1)\n",cpu_getpreviouspc(),input_data,main_ram_seg + dsp_addr_w);
	#endif
		return input_data;
	} };
	
	public static ReadHandlerPtr fsharkbt_dsp_in = new ReadHandlerPtr() { public int handler(int offset)
	{
		...
		fsharkbt_8741 += 1;
	#if LOG_DSP_CALLS
		if (errorlog != 0) fprintf(errorlog,"DSP PC:%04x IO read %04x from 8741 MCU (port 2)\n",cpu_getpreviouspc(),(fsharkbt_8741 & 0x08));
	#endif
	...
	#if LOG_DSP_CALLS
			if (errorlog != 0) fprintf(errorlog,"DSP PC:%04x IO write %04x (%08x) at port 0\n",cpu_getpreviouspc(),data,main_ram_seg + dsp_addr_w);
	#endif
		}
		if (fnction == 1) {
	...
	#if LOG_DSP_CALLS
			if (errorlog != 0) fprintf(errorlog,"DSP PC:%04x IO write %04x at %08x (port 1)\n",cpu_getpreviouspc(),data,main_ram_seg + dsp_addr_w);
	#endif
		}
		if (fnction == 2) {
			/* Flying Shark bootleg DSP writes data to an extra MCU (8741) at IO port 2 */
	#if 0
			if (errorlog != 0) fprintf(errorlog,"DSP PC:%04x IO write from DSP RAM:%04x to 8741 MCU (port 2)\n",cpu_getpreviouspc(),fsharkbt_8741);
	#endif
		}
		if (fnction == 3) {
			...
			/*				communication to main processor*/
	#if LOG_DSP_CALLS
			if (errorlog != 0) fprintf(errorlog,"DSP PC:%04x IO write %04x at port 3\n",cpu_getpreviouspc(),data);
	#endif
			if ((data & 0x8000) != 0) {
			...
			if (data == 0) {
				if (dsp_execute != 0) {
	#if LOG_DSP_CALLS
					if (errorlog != 0) fprintf(errorlog,"Turning %s on\n",toaplan_cpu_type[toaplan_main_cpu]);
	#endif
	...
	{
	#if LOG_DSP_CALLS
		if (errorlog != 0) if (offset < 10) fprintf(errorlog,"%s:%08x write %08x at %08x\n",toaplan_cpu_type[toaplan_main_cpu],cpu_get_pc(),data,0x30000+offset);
	#endif
	...
	public static WriteHandlerPtr twincobr_7800c_w = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
	#if 0
		if (errorlog != 0) fprintf(errorlog,"%s:%08x  Writing %08x to %08x.\n",toaplan_cpu_type[toaplan_main_cpu],cpu_get_pc(),data,toaplan_port_type[toaplan_main_cpu] - offset);
	#endif
	...
	#if LOG_DSP_CALLS
							if (errorlog != 0) fprintf(errorlog,"Turning DSP on and %s off\n",toaplan_cpu_type[toaplan_main_cpu]);
	#endif
	...
	#if LOG_DSP_CALLS
							if (errorlog != 0) fprintf(errorlog,"Turning DSP off\n");
	#endif
	...
	public static ReadHandlerPtr twincobr_sharedram_r = new ReadHandlerPtr() { public int handler(int offset)
	{
		return twincobr_sharedram[offset / 2];
	} };
	
	public static WriteHandlerPtr twincobr_sharedram_w = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
	...
	#endif
		switch (data) {
			case 0x08: if (coin_count != 0) { coin_counter_w(0,1); coin_counter_w(0,0); } break;
			case 0x09: if (coin_count != 0) { coin_counter_w(2,1); coin_counter_w(2,0); } break;
			case 0x0a: if (coin_count != 0) { coin_counter_w(1,1); coin_counter_w(1,0); } break;
			case 0x0b: if (coin_count != 0) { coin_counter_w(3,1); coin_counter_w(3,0); } break;
