...
public class mathbox
{
	...
	public static WriteHandlerPtr mb_go = new WriteHandlerPtr() { public void handler(int addr, int data)
	{
	...
	#ifdef MB_TEST
	  if (errorlog != 0) fprintf (errorlog, "math box command %02x data %02x  ", addr, data);
	#endif
	  ...
	  switch (addr)
	    ...
	    case 0x1f:
	      if (errorlog != 0) fprintf (errorlog, "math box function 0x1f\n");
	      /* $$$ do some computation here (selftest? signature analysis? */
	      break;
	    }
	
	#ifdef MB_TEST
	  if (errorlog != 0) fprintf (errorlog, "  result %04x\n", mb_result & 0xffff);
	#endif
	} };
