...
public class mcr
{
	...
	static public static WriteHandlerPtr subtract_from_counter = new WriteHandlerPtr() { public void handler(int counter, int count);
	
	static void mcr68_493_callback(int param);
	static void zwackery_493_callback(int param);
	
	static public static WriteHandlerPtr zwackery_pia_2_w = new WriteHandlerPtr() { public void handler(int offset, int data);
	static public static WriteHandlerPtr zwackery_pia_3_w = new WriteHandlerPtr() { public void handler(int offset, int data);
	static public static WriteHandlerPtr zwackery_ca2_w = new WriteHandlerPtr() { public void handler(int offset, int data);
	...
	extern public static ReadHandlerPtr zwackery_port_2_r = new ReadHandlerPtr() { public int handler(int offset);
	...
	public static InterruptPtr mcr68_interrupt = new InterruptPtr() { public int handler() 
	{
		...
		if (errorlog != 0) fprintf(errorlog, "--- VBLANK ---\n");
	...
	static void mcr68_493_callback(int param)
	{
		...
		if (errorlog != 0) fprintf(errorlog, "--- (INT1) ---\n");
	}
	...
	public static WriteHandlerPtr mcr_dummy_w = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
	} };
	
	
	public static WriteHandlerPtr mcr_port_01_w = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
		mcr_cocktail_flip = (data >> 6) & 1;
	} };
	
	
	public static WriteHandlerPtr mcr_port_47_dispatch_w = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
	...
	public static ReadHandlerPtr mcr_port_04_dispatch_r = new ReadHandlerPtr() { public int handler(int offset)
	{
		...
		if (m6840_state[counter].control & 0x04)
		{
			...
			while (lsb < 0)
			{
				...
				if (msb < 0)
				{
					...
					if (m6840log != 0) fprintf(m6840log, "** Counter %d fired\n", counter);
				}
			}
	...
	static public static WriteHandlerPtr mcr68_6840_w_common = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
		int i;
		...
		if (offset < 2)
		{
			...
			if (m6840log != 0) fprintf(m6840log, "%06X:Counter %d control = %02X\n", cpu_getpreviouspc(), counter, data);
		}
	
		/* offsets 2, 4, and 6 are MSB buffer registers */
		else if ((offset & 1) == 0)
		{
			if (m6840log != 0) fprintf(m6840log, "%06X:MSB = %02X\n", cpu_getpreviouspc(), data);
		...
		else
		{
			...
			if (m6840log != 0) fprintf(m6840log, "%06X:Counter %d latch = %04X\n", cpu_getpreviouspc(), counter, m6840_state[counter].latch);
		}
	} };
	
	
	static public static ReadHandlerPtr mcr68_6840_r_common = new ReadHandlerPtr() { public int handler(int offset)
	{
		/* offset 0 is a no-op */
		if (offset == 0)
			return 0;
		...
		{
			if (m6840log != 0) fprintf(m6840log, "%06X:Status read = %04X\n", cpu_getpreviouspc(), m6840_status);
		...
		else if ((offset & 1) == 0)
		{
			...
			if (m6840log != 0) fprintf(m6840log, "%06X:Counter %d read = %04X\n", cpu_getpreviouspc(), counter, result);
			return result >> 8;
		}
