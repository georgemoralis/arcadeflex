...
public class atarigen
{
	...
	static void delayed_sound_w(int param)
	{
		...
		if (atarigen_cpu_to_sound_ready != 0)
			if (errorlog != 0) fprintf(errorlog, "Missed command from 68010\n");
	...
	static void delayed_6502_sound_w(int param)
	{
		...
		if (atarigen_sound_to_cpu_ready != 0)
			if (errorlog != 0) fprintf(errorlog, "Missed result from 6502\n");
	...
	/* prototypes */
	static public static ReadHandlerPtr m6502_speedup_r = new ReadHandlerPtr() { public int handler(int offset);
	...
	void atarigen_init_6502_speedup(int cpunum, int compare_pc1, int compare_pc2)
	{
		...
		if (address_low != address_high - 1)
			if (errorlog != 0) fprintf(errorlog, "Error: address %04X does not point to a speedup location!", compare_pc1);
		...
		if (address_low != address_high - 1)
			if (errorlog != 0) fprintf(errorlog, "Error: address %04X does not point to a speedup location!", compare_pc2);
	...
	{
		if (errorlog != 0) fprintf(errorlog, "video_control_r(%02X)\n", offset);
	...
	void atarigen_mo_update(const UINT8 *base, int link, int scanline)
	{
		...
		while (!spritevisit[link])
		{
			...
			{
				if (errorlog != 0) fprintf(errorlog, "Motion object list exceeded maximum\n");
				break;
			}
	...
	static public static ReadHandlerPtr compute_shift = new ReadHandlerPtr() { public int handler(int size);
	static public static ReadHandlerPtr compute_mask = new ReadHandlerPtr() { public int handler(int count);
	...
	void atarigen_pf_free(void)
	{
		...
		if (atarigen_pf_overrender_bitmap != 0)
