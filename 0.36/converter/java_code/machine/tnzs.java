...
public class tnzs
{
	...
	public static ReadHandlerPtr arkanoi2_sh_f000_r = new ReadHandlerPtr() { public int handler(int offset)
	{
		...
		if (errorlog != 0) fprintf (errorlog, "PC %04x: read input %04x\n", cpu_get_pc(), 0xf000 + offset);
	...
	static void mcu_handle_coins(int coin)
	{
		...
		else if (coin && coin != insertcoin)
		{
			if ((coin & 0x01) != 0)	/* coin A */
			{
				...
				{
					if (errorlog != 0) fprintf (errorlog, "Coin dropped into slot A\n");
			...
			if ((coin & 0x02) != 0)	/* coin B */
			{
				...
				{
					if (errorlog != 0) fprintf (errorlog, "Coin dropped into slot B\n");
					coin_lockout_global_w(0,0); /* Unlock all coin slots */
			...
			{
				if (errorlog != 0) fprintf (errorlog, "Coin dropped into service slot C\n");
				mcu_credits++;
			}
			mcu_reportcoin = coin;
		}
	...
	static public static ReadHandlerPtr mcu_arkanoi2_r = new ReadHandlerPtr() { public int handler(int offset)
	{
	...
	//	if (errorlog != 0) fprintf (errorlog, "PC %04x: read mcu %04x\n", cpu_get_pc(), 0xc000 + offset);
		...
		if (offset == 0)
		{
			...
			switch (mcu_command)
			{
				...
				default:
	if (errorlog != 0) fprintf (errorlog, "error, unknown mcu command\n");
					/* should not happen */
					return 0xff;
					break;
			}
	...
	static public static WriteHandlerPtr mcu_arkanoi2_w = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
		if (offset == 0)
		{
	...
	static public static ReadHandlerPtr mcu_chukatai_r = new ReadHandlerPtr() { public int handler(int offset)
	{
		char *mcu_startup = "\xa5\x5a\xaa";
	
		if (errorlog != 0) fprintf (errorlog, "PC %04x (re %04x): read mcu %04x\n", cpu_get_pc(), cpu_geturnpc(), 0xc000 + offset);
		...
		if (offset == 0)
		{
			...
			switch (mcu_command)
			{
				...
				default:
	if (errorlog != 0) fprintf (errorlog, "error, unknown mcu command (%02x)\n",mcu_command);
					/* should not happen */
					return 0xff;
					break;
			}
	...
	static public static WriteHandlerPtr mcu_chukatai_w = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
		if (errorlog != 0) fprintf (errorlog, "PC %04x (re %04x): write %02x to mcu %04x\n", cpu_get_pc(), cpu_geturnpc(), data, 0xc000 + offset);
	...
	static public static ReadHandlerPtr mcu_tnzs_r = new ReadHandlerPtr() { public int handler(int offset)
	{
	...
	static public static WriteHandlerPtr mcu_tnzs_w = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
		if (offset == 0)
		{
			if (errorlog != 0) fprintf (errorlog, "PC %04x (re %04x): write %02x to mcu %04x\n", cpu_get_pc(), cpu_geturnpc(), data, 0xc000 + offset);
