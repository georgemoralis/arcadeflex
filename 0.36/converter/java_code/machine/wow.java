...
public class wow
{
	...
	#ifdef MAME_DEBUG
	        if (errorlog != 0) fprintf(errorlog,"Gorf Delay set to %02x\n",GorfDelay);
	#endif
	
	    }
	
	#ifdef MAME_DEBUG
	    if (errorlog != 0) fprintf(errorlog,"Interrupt Flag set to %02x\n",InterruptFlag);
	#endif
	} };
	...
	#ifdef MAME_DEBUG
		if (errorlog != 0) fprintf(errorlog,"Scanline interrupt set to %02x\n",data);
	#endif
	...
	public static ReadHandlerPtr ebases_trackball_r = new ReadHandlerPtr() { public int handler(int offset)
	{
		...
		if (errorlog != 0) fprintf(errorlog, "Port %d = %d\n", ebases_trackball_select, ret);
		return ret;
	} };
}
