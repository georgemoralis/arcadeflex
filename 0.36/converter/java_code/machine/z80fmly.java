...
public class z80fmly
{
	...
	double z80ctc_getperiod (int which, int ch)
	{
		...
		{
			if (errorlog != 0) fprintf (errorlog, "CTC %d is CounterMode : Can't calcrate period\n", ch );
			return 0;
		}
	...
	void z80ctc_w (int which, int offset, int data)
	{
	...
	#endif
		{
			...
			if (errorlog != 0) fprintf (errorlog, "CTC Vector = %02x\n", ctc.vector);
			return;
		}
		...
		if ((data & CONTROL) == CONTROL_WORD)
		{
			/* set the new mode */
			ctc.mode[ch] = data;
			if (errorlog != 0) fprintf (errorlog,"CTC ch.%d mode = %02x\n", ch, data);
	...
	if (errorlog != 0) fprintf(errorlog,"CTC clock %f\n",1.0/clock);
	...
	public static ReadHandlerPtr z80ctc_interrupt = new ReadHandlerPtr() { public int handler(int which)
	{
		z80ctc *ctc = ctcs + which;
		int ch;
		...
		if( ch > 3 )
		{
			if (errorlog != 0) fprintf(errorlog,"CTC entry INT : non IRQ\n");
			ch = 0;
		}
		z80ctc_interrupt_check( ctc );
	...
	void z80pio_d_w( int which , int ch , int data )
	{
		...
		case PIO_MODE3:			/* mode 0 bit */
			return;
		default:
			if (errorlog != 0) fprintf(errorlog,"PIO-%c data write,bad mode\n",'A'+ch );
		}
	}
	...
	void z80pio_c_w( int which , int ch , int data )
	{
		...
		if( pio.enable[ch] & PIO_INT_MASK ){	/* load mask folows */
			...
			if (errorlog != 0) fprintf(errorlog,"PIO-%c interrupt mask %02x\n",'A'+ch,data );
			return;
		}
		switch( data & 0x0f ){
		case PIO_OP_MODE:	/* mode select 0=out,1=in,2=i/o,3=bit */
			...
			if (errorlog != 0) fprintf(errorlog,"PIO-%c Mode %x\n",'A'+ch,pio.mode[ch] );
			break;
		case PIO_OP_INTC:		/* interrupt control */
			pio.enable[ch] = data & 0xf0;
			pio.mask[ch]   = 0x00;
			/* when interrupt enable , set vector request flag */
			if (errorlog != 0) fprintf(errorlog,"PIO-%c Controll %02x\n",'A'+ch,data );
			break;
		case PIO_OP_INTE:		/* interrupt enable controll */
			pio.enable[ch] &= ~PIO_INT_ENABLE;
			pio.enable[ch] |= (data & PIO_INT_ENABLE);
			if (errorlog != 0) fprintf(errorlog,"PIO-%c enable %02x\n",'A'+ch,data&0x80 );
			break;
		default:
				if( !(data&1) )
				{
					pio.vector[ch] = data;
					if (errorlog != 0) fprintf(errorlog,"PIO-%c vector %02x\n",'A'+ch,data);
				}
				else if (errorlog != 0) fprintf(errorlog,"PIO-%c illegal command %02x\n",'A'+ch,data );
		}
		/* interrupt check */
		z80pio_check_irq( pio , ch );
	}
	...
	int z80pio_c_r( int which , int ch )
	{
		...
		if (errorlog != 0) fprintf(errorlog,"PIO-%c controll read\n",'A'+ch );
		return 0;
	}
	...
	int z80pio_d_r( int which , int ch )
	{
		...
		case PIO_MODE1:			/* mode 1 intput */
			...
			return pio.in[ch];
		case PIO_MODE2:			/* mode 2 i/o */
			if (ch != 0) if (errorlog != 0) fprintf(errorlog,"PIO-B mode 2 \n");
			pio.rdy[1] = 1;	/* brdy = H */
			z80pio_check_irq( pio , ch );
			return pio.in[ch];
		...
		if (errorlog != 0) fprintf(errorlog,"PIO-%c data read,bad mode\n",'A'+ch );
		return 0;
	}
	
	public static ReadHandlerPtr z80pio_interrupt = new ReadHandlerPtr() { public int handler(int which)
	{
		z80pio *pio = pios + which;
		int ch = 0;
		...
		} if( pio.int_state[0] == 0 )
		{
			...
			{
				if (errorlog != 0) fprintf(errorlog,"PIO entry INT : non IRQ\n");
				ch = 0;
			}
		}
		z80pio_interrupt_check( pio );
	...
	void z80pio_p_w( int which , int ch , int data )
	{
		...
		switch( pio.mode[ch] ){
		case PIO_MODE0:
			if (errorlog != 0) fprintf(errorlog,"PIO-%c OUTPUT mode and data write\n",'A'+ch );
			break;
	...
	int z80pio_p_r( int which , int ch )
	{
		...
		case PIO_MODE0:
			...
			break;
		case PIO_MODE1:
			if (errorlog != 0) fprintf(errorlog,"PIO-%c INPUT mode and data read\n",'A'+ch );
			break;
		case PIO_MODE3:
