...
public class namcos1
{
	...
	/* from vidhrdw */
	extern public static ReadHandlerPtr namcos1_videoram_r = new ReadHandlerPtr() { public int handler(int offset);
	extern public static WriteHandlerPtr namcos1_videoram_w = new WriteHandlerPtr() { public void handler(int offset, int data);
	extern public static ReadHandlerPtr namcos1_paletteram_r = new ReadHandlerPtr() { public int handler(int offset);
	extern public static WriteHandlerPtr namcos1_paletteram_w = new WriteHandlerPtr() { public void handler(int offset, int data);
	extern public static WriteHandlerPtr namcos1_videocontrol_w = new WriteHandlerPtr() { public void handler(int offset, int data);
	...
	extern public static WriteHandlerPtr namcos1_set_sprite_offsets = new WriteHandlerPtr() { public void handler(int x, int y);
	...
	*******************************************************************************/
	...
	static public static ReadHandlerPtr rev1_key_r = new ReadHandlerPtr() { public int handler(int offset){
	//	if (errorlog != 0) fprintf(errorlog,"CPU #%d PC %08x: keychip read %04X=%02x\n",cpu_getactivecpu(),cpu_get_pc(),offset,key[offset]);
		if(offset >= NAMCOS1_MAX_KEY)
		{
			if (errorlog != 0) fprintf(errorlog,"CPU #%d PC %08x: unmapped keychip read %04x\n",cpu_getactivecpu(),cpu_get_pc(),offset);
			return 0;
		}
		return key[offset];
	} };
	
	static public static WriteHandlerPtr rev1_key_w = new WriteHandlerPtr() { public void handler(int offset, int data){
		static unsigned short divider, divide_32 = 0;
		//if (errorlog != 0) fprintf(errorlog,"CPU #%d PC %08x: keychip write %04X=%02x\n",cpu_getactivecpu(),cpu_get_pc(),offset,data);
		if(offset >= NAMCOS1_MAX_KEY)
		{
			if (errorlog != 0) fprintf(errorlog,"CPU #%d PC %08x: unmapped keychip write %04x=%04x\n",cpu_getactivecpu(),cpu_get_pc(),offset,data);
			return;
		}
	...
	*******************************************************************************/
	...
	static public static ReadHandlerPtr rev2_key_r = new ReadHandlerPtr() { public int handler(int offset)
	{
		//if (errorlog != 0) fprintf(errorlog,"CPU #%d PC %08x: keychip read %04X=%02x\n",cpu_getactivecpu(),cpu_get_pc(),offset,key[offset]);
		if(offset >= NAMCOS1_MAX_KEY)
		{
	...
	static public static WriteHandlerPtr rev2_key_w = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
		//if (errorlog != 0) fprintf(errorlog,"CPU #%d PC %08x: keychip write %04X=%02x\n",cpu_getactivecpu(),cpu_get_pc(),offset,data);
		if(offset >= NAMCOS1_MAX_KEY)
		{
	...
	*******************************************************************************/
	...
	static public static ReadHandlerPtr dangseed_key_r = new ReadHandlerPtr() { public int handler(int offset){
	//	if (errorlog != 0) fprintf(errorlog,"CPU #%d PC %08x: keychip read %04X=%02x\n",cpu_getactivecpu(),cpu_get_pc(),offset,key[offset]);
		if(offset >= NAMCOS1_MAX_KEY)
		{
			if (errorlog != 0) fprintf(errorlog,"CPU #%d PC %08x: unmapped keychip read %04x\n",cpu_getactivecpu(),cpu_get_pc(),offset);
	...
	static public static WriteHandlerPtr dangseed_key_w = new WriteHandlerPtr() { public void handler(int offset, int data){
		int i;
	//	if (errorlog != 0) fprintf(errorlog,"CPU #%d PC %08x: keychip write %04X=%02x\n",cpu_getactivecpu(),cpu_get_pc(),offset,data);
		if(offset >= NAMCOS1_MAX_KEY)
		{
	...
	*******************************************************************************/
	...
	static public static ReadHandlerPtr dspirit_key_r = new ReadHandlerPtr() { public int handler(int offset)
	{
		//if (errorlog != 0) fprintf(errorlog,"CPU #%d PC %08x: keychip read %04X=%02x\n",cpu_getactivecpu(),cpu_get_pc(),offset,key[offset]);
		if(offset >= NAMCOS1_MAX_KEY)
		{
	...
	static public static WriteHandlerPtr dspirit_key_w = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
	...
	*******************************************************************************/
	...
	static public static ReadHandlerPtr blazer_key_r = new ReadHandlerPtr() { public int handler(int offset)
	{
		if (errorlog != 0) fprintf(errorlog,"CPU #%d PC %08x: keychip read %04X=%02x\n",cpu_getactivecpu(),cpu_get_pc(),offset,key[offset]);
		if(offset >= NAMCOS1_MAX_KEY)
		{
	...
	static public static WriteHandlerPtr blazer_key_w = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
		static unsigned short divisor;
		if (errorlog != 0) fprintf(errorlog,"CPU #%d PC %08x: keychip write %04X=%02x\n",cpu_getactivecpu(),cpu_get_pc(),offset,data);
		if(offset >= NAMCOS1_MAX_KEY)
	...
	*******************************************************************************/
	...
	static public static ReadHandlerPtr ws_key_r = new ReadHandlerPtr() { public int handler(int offset){
	//	if (errorlog != 0) fprintf(errorlog,"CPU #%d PC %08x: keychip read %04X=%02x\n",cpu_getactivecpu(),cpu_get_pc(),offset,key[offset]);
		if(offset >= NAMCOS1_MAX_KEY)
		{
			if (errorlog != 0) fprintf(errorlog,"CPU #%d PC %08x: unmapped keychip read %04x\n",cpu_getactivecpu(),cpu_get_pc(),offset);
	...
	static public static WriteHandlerPtr ws_key_w = new WriteHandlerPtr() { public void handler(int offset, int data){
	...
	*******************************************************************************/
	...
	static public static ReadHandlerPtr soundram_r = new ReadHandlerPtr() { public int handler(int offset)
	{
		if(offset<0x100)
	...
	static public static WriteHandlerPtr soundram_w = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
		if(offset<0x100)
		{
	...
	static public static WriteHandlerPtr rom_w = new WriteHandlerPtr() { public void handler(int offset, int data){
		if (errorlog != 0)
			fprintf(errorlog,"CPU #%d PC %04x: warning - write %02x to rom address %04x\n",cpu_getactivecpu(),cpu_get_pc(),data,offset);
	} };
	
	/* error handlers */
	static public static ReadHandlerPtr unknown_r = new ReadHandlerPtr() { public int handler(int offset){
		if (errorlog != 0)
			fprintf(errorlog,"CPU #%d PC %04x: warning - read from unknown chip\n",cpu_getactivecpu(),cpu_get_pc() );
		return 0;
	} };
	
	static public static WriteHandlerPtr unknown_w = new WriteHandlerPtr() { public void handler(int offset, int data){
		if (errorlog != 0)
	...
	public static WriteHandlerPtr namcos1_subcpu_bank = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
		int oldcpu = cpu_getactivecpu();
	
		//if (errorlog != 0) fprintf(errorlog,"cpu1 bank selected %02x=%02x\n",offset,data);
	...
	public static ReadHandlerPtr namcos1_##cpu##_banked_area##bank##_r = new ReadHandlerPtr() { public int handler(int offset){\
	...
	public static WriteHandlerPtr namcos1_##cpu##_banked_area##bank##_w = new WriteHandlerPtr() { public void handler(int offset, int data){\
	...
	*******************************************************************************/
	...
	{
	//	if (errorlog != 0) fprintf(errorlog,"reset control pc=%04x %02x\n",cpu_get_pc(),data);
	...
	*******************************************************************************/
	...
	/* sound cpu */
	static public static ReadHandlerPtr namcos1_sound_spinlock_r = new ReadHandlerPtr() { public int handler(int offset)
	{
	...
	*******************************************************************************/
	...
	{
	extern public static WriteHandlerPtr mwh_bank3 = new WriteHandlerPtr() { public void handler(int _address, int _data);
	
		//if (errorlog != 0) fprintf(errorlog,"mcu C000 write pc=%04x data=%02x\n",cpu_get_pc(),data);
	...
	*******************************************************************************/
	...
	static public static ReadHandlerPtr namcos1_setopbase_0 = new ReadHandlerPtr() { public int handler(int pc)
	{
		int bank = (pc>>13)&7;
	...
	static public static ReadHandlerPtr namcos1_setopbase_1 = new ReadHandlerPtr() { public int handler(int pc){
		int bank = (pc>>13)&7;
	...
	*******************************************************************************/
	static public static ReadHandlerPtr berabohm_buttons_r = new ReadHandlerPtr() { public int handler(int offset)
	{
		int res;
