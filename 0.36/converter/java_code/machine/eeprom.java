...
public class eeprom
{
	...
	{
	#if VERBOSE
	if (errorlog != 0) fprintf(errorlog,"EEPROM write bit %d\n",bit);
	#endif
	
		if (serial_count >= SERIAL_BUFFER_LENGTH-1)
		{
	if (errorlog != 0) fprintf(errorlog,"error: EEPROM serial buffer overflow\n");
			return;
		}
				...
				!strncmp(serial_buffer,intf.cmd_read,strlen(intf.cmd_read)))
		{
			...
			sending = 1;
			serial_count = 0;
	if (errorlog != 0) fprintf(errorlog,"EEPROM read %04x from address %02x\n",eeprom_data_bits,address);
		}
	...
	if (errorlog != 0) fprintf(errorlog,"EEPROM erase address %02x\n",address);
			if (locked == 0)
			{
				if (intf.data_bits == 16)
				{
	...
	if (errorlog != 0) fprintf(errorlog,"Error: EEPROM is locked\n");
			serial_count = 0;
		}
	...
	if (errorlog != 0) fprintf(errorlog,"EEPROM write %04x to address %02x\n",data,address);
			if (locked == 0)
			{
				if (intf.data_bits == 16)
				{
	...
	if (errorlog != 0) fprintf(errorlog,"Error: EEPROM is locked\n");
		...
		{
	if (errorlog != 0) fprintf(errorlog,"EEPROM lock\n");
			locked = 1;
			serial_count = 0;
		}
		...
		{
	if (errorlog != 0) fprintf(errorlog,"EEPROM unlock\n");
			locked = 0;
			serial_count = 0;
		}
	}
	...
	{
	#if VERBOSE
	if (errorlog != 0) fprintf(errorlog,"write bit %d\n",bit);
	#endif
		latch = bit;
	}
	...
	#if VERBOSE
	if (errorlog != 0) fprintf(errorlog,"read bit %d\n",res);
	#endif
	...
	{
	#if VERBOSE
	if (errorlog != 0) fprintf(errorlog,"set reset line %d\n",state);
	#endif
		reset_line = state;
	...
	{
	#if VERBOSE
	if (errorlog != 0) fprintf(errorlog,"set clock line %d\n",state);
	#endif
