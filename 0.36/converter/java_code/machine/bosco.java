...
public class bosco
{
	...
	***************************************************************************/
	...
	if (errorlog != 0) fprintf(errorlog,"%04x: custom IO 1 offset %02x data %02x\n",cpu_get_pc(),offset,data);
	...
	if (errorlog && data != 0x10) fprintf(errorlog,"%04x: custom IO 1 command %02x\n",cpu_get_pc(),data);
		...
		{
			case 0x10:
				if (nmi_timer_1 != 0) timer_remove (nmi_timer_1);
				nmi_timer_1 = 0;
				return;
	...
	***************************************************************************/
	...
	if (errorlog != 0) fprintf(errorlog,"%04x: custom IO 2 offset %02x data %02x\n",cpu_get_pc(),offset,data);
	...
	if (errorlog && data != 0x10) fprintf(errorlog,"%04x: custom IO 2 command %02x\n",cpu_get_pc(),data);
		...
		{
			case 0x10:
				if (nmi_timer_2 != 0) timer_remove (nmi_timer_2);
				nmi_timer_2 = 0;
				return;
		}
