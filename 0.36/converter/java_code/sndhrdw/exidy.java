...
public class exidy
{
	...
	***************************************************************************/
	...
	public static ShStopPtr exidy_sh_stop = new ShStopPtr() { public void handler() 
	{
	...
	***************************************************************************/
	...
	static void riot_interrupt(int parm)
	{
	    if (riot_state == RIOT_COUNTUP) {
	        ...
	        if (irq_enable != 0) cpu_cause_interrupt (1, M6502_INT_IRQ);
	...
	public static WriteHandlerPtr exidy_shriot_w = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
	   ...
	   case 0:
	    ...
	    case 0x1f:
			...
			return;
		default:
		    if (errorlog != 0) fprintf(errorlog,"Undeclared RIOT write: %x=%x\n",offset,data);
		    return;
		}
	...
	public static ReadHandlerPtr exidy_shriot_r = new ReadHandlerPtr() { public int handler(int offset)
	{
		...
		default:
		    if (errorlog != 0) fprintf(errorlog,"Undeclared RIOT read: %x  PC:%x\n",offset,cpu_get_pc());
	  	    return 0xff;
		}
		return 0;
	} };
	...
	***************************************************************************/
	...
	{
	    if (errorlog != 0) fprintf(errorlog,"8253(R): %x\n",offset);
		return 0;
	} };
	...
	***************************************************************************/
	...
	public static ReadHandlerPtr exidy_sh6840_r = new ReadHandlerPtr() { public int handler(int offset){
	    if (errorlog != 0) fprintf(errorlog,"6840R %x\n",offset);
	    return 0;
	} };
