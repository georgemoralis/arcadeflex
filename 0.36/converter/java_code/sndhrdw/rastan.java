...
public class rastan
{
	...
	*/
	...
	public static ReadHandlerPtr r_rd_a001 = new ReadHandlerPtr() { public int handler(int offset)
	{
		...
		if (transmit == 0)
		{
			if (errorlog != 0) fprintf(errorlog,"Slave unexpected receiving! (PC = %04x)\n", cpu_get_pc() );
		}
		else
		{
			if (tr_mode == 1)
			{
				...
				if (errorlog != 0) fprintf(errorlog,"Slave has read status of itself %02x (PC = %04x)\n",pom, cpu_get_pc() );
	#endif
			}
			else
			{            /*2-bytes transmision*/
				if (transmit==2)
				{
					...
					if (errorlog != 0) fprintf(errorlog,"Slave has read pom1=%02x (PC = %04x)\n",pom, cpu_get_pc() );
	#endif
				}
				else
				{
	...
	#ifdef REPORT_DATA_FLOW
					if (errorlog != 0) fprintf(errorlog,"Slave has read pom2=%02x (PC = %04x)\n",pom,cpu_get_pc() );
	#endif
	...
	public static WriteHandlerPtr r_wr_a000 = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
		...
		if (transmit != 0)
			if (errorlog != 0) fprintf(errorlog,"Slave mode changed while expecting to transmit! (PC = %04x) \n", cpu_get_pc() );
	
	#ifdef REPORT_SLAVE_MODE_CHANGE
		if (errorlog != 0) fprintf(errorlog,"Slave changing its mode to %02x (PC = %04x) \n",data, cpu_get_pc());
	#endif
		...
		if (pom == 0x03)
			if (errorlog != 0) fprintf(errorlog,"Int mode = 3! (PC = %04x)\n", cpu_get_pc() );
	} };
	...
	public static WriteHandlerPtr r_wr_a001 = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
		...
		if (transmit == 0)
		{
			if (errorlog != 0) fprintf(errorlog,"Slave unexpected transmission! (PC = %04x)\n", cpu_get_pc() );
		}
		else
		{
			if (transmit == 2)
	...
	#ifdef REPORT_DATA_FLOW
					if (errorlog != 0) fprintf(errorlog,"Slave sent double %02x (PC = %04x)\n",lasthalf+(data<<4), cpu_get_pc() );
	#endif
				}
				else
				{
	#ifdef REPORT_DATA_FLOW
					if (errorlog != 0) fprintf(errorlog,"Slave issued control value %02x (PC = %04x)\n",data, cpu_get_pc() );
	#endif
				}
			}
		}
	...
	public static WriteHandlerPtr rastan_sound_comm_w = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
		...
		if (m_transmit == 0)
		{
			if (errorlog != 0) fprintf(errorlog,"Main unexpected transmission! (PC = %08x)\n", cpu_get_pc() );
		}
		else
		{
			if (m_transmit == 2)
	...
	#ifdef REPORT_DATA_FLOW
					if (errorlog != 0) fprintf(errorlog,"Main sent double %02x (PC = %08x) \n",m_lasthalf+(data<<4), cpu_get_pc() );
	#endif
				}
				else
				{
	#ifdef REPORT_DATA_FLOW
					if (errorlog != 0) fprintf(errorlog,"Main issued control value %02x (PC = %08x) \n",data, cpu_get_pc() );
	#endif
	...
	public static ReadHandlerPtr rastan_sound_comm_r = new ReadHandlerPtr() { public int handler(int offset)
	{
		...
		{
	#ifdef REPORT_DATA_FLOW
			if (errorlog != 0) fprintf(errorlog,"Main read double %02x (PC = %08x)\n",soundboarddata, cpu_get_pc() );
	#endif
		...
		else
		{
			...
			if (errorlog != 0) fprintf(errorlog,"Main read status of Slave %02x (PC = %08x)\n",SlaveContrStat, cpu_get_pc() );
	#endif
			m_transmit++;
