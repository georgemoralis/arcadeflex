...
public class wow
{
	...
	public static ReadHandlerPtr wow_speech_r = new ReadHandlerPtr() { public int handler(int offset)
	{
	    ...
	    if (errorlog != 0) fprintf(errorlog,"Data : %d Speech : %s at intonation %d\n",Phoneme, PhonemeTable[Phoneme],Intonation);
	
	    if(Phoneme==63) {
	   		sample_stop(wowChannel);
	                if (errorlog != 0) fprintf(errorlog,"Clearing sample %s\n",totalword);
	...
	/* Phoneme to word translation */
	    ...
	    if (strlen(totalword) == 0) {
	       ...
	       if (plural != 0) {
	          if (errorlog != 0) fprintf(errorlog,"found a possible plural at %d\n",plural-1);
	    ...
	    if (errorlog != 0) fprintf(errorlog,"Total word = %s\n",totalword);
	    ...
	    for (i=0; wowWordTable[i]; i++) {
		  ...
		  /* WOW has Dungeon */
		     ...
		     if (errorlog != 0) fprintf(errorlog,"Storing sample position %d and copying string %s\n",plural,oldword);
	          } else {
	             plural=0;
	          }
	          sample_start(wowChannel, i, 0);	                   /* play the sample at position of word */
	          sample_set_freq(wowChannel, wowBaseFrequency);         /* play at correct rate */
	          if (errorlog != 0) fprintf(errorlog,"Playing sample %d",i);
	          totalword[0] = 0;				   /* Clear the total word stack */
	          return data;
	       }
	    }
	...
	{
	    if (errorlog != 0) fprintf(errorlog, "asked for samples status %d\n",wowChannel);
