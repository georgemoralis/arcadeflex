...
public class cinemat
{
	...
	***************************************************************************/
	...
	public static VhStartPtr cinemat_vh_start = new VhStartPtr() { public int handler() 
	{
		...
		if (overlay != 0) overlay_remap (overlay);
	...
	{
		vector_set_shift (VEC_SHIFT);
		if (spacewar_panel != 0) backdrop_refresh(spacewar_panel);
		if (spacewar_pressed_panel != 0) backdrop_refresh(spacewar_pressed_panel);
		cinemat_screenh = Machine.drv.visible_area.max_y - Machine.drv.visible_area.min_y;
		return vector_vh_start();
	} };
	...
	public static VhStopPtr cinemat_vh_stop = new VhStopPtr() { public void handler() 
	{
		...
		if (overlay != 0) artwork_free (overlay);
		vector_vh_stop();
	} };
