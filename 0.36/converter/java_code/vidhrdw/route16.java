...
public class route16
{
	...
	/* Local functions */
	static public static WriteHandlerPtr modify_pen = new WriteHandlerPtr() { public void handler(int pen, int colorindex);
	static void common_videoram_w(int offset,int data,
	                              int coloroffset, struct osd_bitmap *bitmap);
	
	
	
	public static VhConvertColorPromPtr route16_vh_convert_color_prom = new VhConvertColorPromPtr() { public void handler(UByte []palette, char []colortable, UBytePtr color_prom) 
	{
	...
	***************************************************************************/
	public static VhUpdatePtr route16_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
	    if (video_remap_1 != 0)
		{
