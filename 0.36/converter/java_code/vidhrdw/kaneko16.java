...
public class kaneko16
{
	...
	*/
	...
	public static WriteHandlerPtr kaneko16_screen_regs_w = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
		...
		if (errorlog != 0) fprintf(errorlog, "CPU #0 PC %06X : Warning, screen reg %04X <- %04X\n",cpu_get_pc(),offset,data);
	} };
	...
	***************************************************************************/
	...
	void kaneko16_draw_sprites(struct osd_bitmap *bitmap, int priority)
	{
		...
		if ( debugsprites && ( ((sattr >> 6) & 3) != debugsprites-1 ) )	continue;
			...
			if ((flipsprites & 2) != 0) { sx = max_x - sx;		sflipx = !sflipx; }
			if ((flipsprites & 1) != 0) { sy = max_y - sy;		sflipy = !sflipy; }
