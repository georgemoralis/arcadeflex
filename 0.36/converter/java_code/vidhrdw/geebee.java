...
public class geebee
{
	...
	public static VhStopPtr geebee_vh_stop = new VhStopPtr() { public void handler() 
	{
		if (overlay != 0)
			free(overlay);
		overlay = NULL;
	...
	INLINE public static WriteHandlerPtr mark_dirty = new WriteHandlerPtr() { public void handler(int x, int y)
	{
		int cx, cy, offs;
		cy = y / 8;
		cx = x / 8;
