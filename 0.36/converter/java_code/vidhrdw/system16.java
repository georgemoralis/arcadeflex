...
public class system16
{
	...
	static public static WriteHandlerPtr get_bg2_tile_info = new WriteHandlerPtr() { public void handler(int col, int row){
		const UINT16 *source = (const UINT16 *)sys16_tileram;
	...
	static public static WriteHandlerPtr get_fg2_tile_info = new WriteHandlerPtr() { public void handler(int col, int row){
		const UINT16 *source = (const UINT16 *)sys16_tileram;
	...
	#ifdef SPACEHARRIER_OFFSETS
		if (spaceharrier_patternoffsets != 0) free(spaceharrier_patternoffsets);
	...
	*/
			while( sprite<finish ){
				...
				if(bottom !=0 && bottom > top)
				{
	...
	#ifdef SYS16_DEBUG
					if (dump_spritedata != 0) fprintf(errorlog,"0:%4x  1:%4x  2:%4x  3:%4x  4:%4x  5:%4x  6:%4x  7:%4x\n",
					...
					if ((width & 0x100) != 0) sprite.flags |= SPRITE_FLIPX;
					if ((width & 0x080) != 0) sprite.flags |= SPRITE_FLIPY;
	...
	*/
			while( sprite<finish ){
				...
				if( bottom>top && ypos!=0xffff ){
					...
					if ((number & 0x8000) != 0) sprite.flags |= SPRITE_FLIPX;
					if ((width & 0x0080) != 0) sprite.flags |= SPRITE_FLIPY;
					sprite.flags |= SPRITE_VISIBLE;
					...
					if( !(width & 0x180) && (width & 0x7f))
					{
						...
						{ // build pattern offset
							...
							if (len_pattern >= 16)
							{
							    ...
							    if (errorlog != 0) fprintf(errorlog,"addr=%04x offset=%4x bank=%02x offset=%d height=%d zoomy=%04x len_pattern=%d width=%d offset_addr=%04x\n", addr, offset, bank, offset, height, zoomy,  len_pattern, width2, offset_addr);
							}
						}
					}
