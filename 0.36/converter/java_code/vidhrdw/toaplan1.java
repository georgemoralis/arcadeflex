...
public class toaplan1
{
	...
	public static VhStartPtr rallybik_vh_start = new VhStartPtr() { public int handler() 
	{
		...
		if (errorlog != 0) fprintf (errorlog, "colorram_size: %08x\n", colorram1_size + colorram2_size);
	...
	public static VhStopPtr rallybik_vh_stop = new VhStopPtr() { public void handler() 
	{
		...
		{
			free(tile_list[i]);
			if (errorlog != 0) fprintf (errorlog, "max_list_size[%d]=%08x\n",i,max_list_size[i]);
		}
	...
	public static WriteHandlerPtr toaplan1_videoram1_w = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
		...
		{
			if (errorlog != 0) fprintf (errorlog, "videoram1_w, %08x\n", 2*video_ofs);
			return;
		}
	#endif
	...
	public static WriteHandlerPtr toaplan1_videoram2_w = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
		...
		{
			if (errorlog != 0) fprintf (errorlog, "videoram2_w, %08x\n", 2*video_ofs);
			return;
		}
	#endif
	...
	public static WriteHandlerPtr toaplan1_videoram3_w = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
		...
		{
			if (errorlog != 0) fprintf (errorlog, "videoram3_w, %08x\n", 2*video_ofs3);
			return;
		}
	#endif
	...
	*/
		...
		if (errorlog != 0) fprintf (errorlog, "layers_offset[0]:%08x\n",layers_offset[0]);
		if (errorlog != 0) fprintf (errorlog, "layers_offset[1]:%08x\n",layers_offset[1]);
		if (errorlog != 0) fprintf (errorlog, "layers_offset[2]:%08x\n",layers_offset[2]);
		if (errorlog != 0) fprintf (errorlog, "layers_offset[3]:%08x\n",layers_offset[3]);
	...
	***************************************************************************/
	...
	static public static WriteHandlerPtr toaplan1_find_tiles = new WriteHandlerPtr() { public void handler(int xoffs, int yoffs)
	{
		int priority;
		int layer;
		tile_struct *tinfo;
	...
	static void toaplan1_find_sprites (void)
	{
		 ...
		 && (keyboard_pressed(KEYCODE_N)) )
		{
			...
			for ( sprite = 0 ; sprite < 256 ; sprite++ )
			{
				...
				if (errorlog != 0) fprintf (errorlog, "%08x: %04x %04x\n", sprite, tattr, tchar);
				s_info += 8 ;
			}
		}
	#endif		// end
