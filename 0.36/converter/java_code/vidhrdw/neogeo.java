...
public class neogeo
{
	...
	#ifdef NEO_DEBUG
	int neo_unknown[32];
	public static WriteHandlerPtr neo_unknown1 = new WriteHandlerPtr() { public void handler(int offset, int data){WRITE_WORD(&neo_unknown[0],data);} };
	public static WriteHandlerPtr neo_unknown2 = new WriteHandlerPtr() { public void handler(int offset, int data){WRITE_WORD(&neo_unknown[2],data);} };
	public static WriteHandlerPtr neo_unknown3 = new WriteHandlerPtr() { public void handler(int offset, int data){WRITE_WORD(&neo_unknown[4],data);} };
	public static WriteHandlerPtr neo_unknown4 = new WriteHandlerPtr() { public void handler(int offset, int data){if (cpu_get_pc()!=0x4a44) WRITE_WORD(&neo_unknown[6],data>>7);} };
	...
	{
	   	if (pal_bank1 != 0) free (pal_bank1);
		if (pal_bank2 != 0) free (pal_bank2);
		if (vidram != 0) free (vidram);
		if (neogeo_ram != 0) free (neogeo_ram);
	...
	public static WriteHandlerPtr neogeo_setpalbank0 = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
		if (palno != 0) {
			palno = 0;
			neogeo_paletteram = pal_bank1;
	...
	public static WriteHandlerPtr neogeo_setpalbank1 = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
		if (palno != 1) {
			palno = 1;
	...
	public static WriteHandlerPtr neo_board_fix = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
		fix_bank=1;
	} };
	
	public static WriteHandlerPtr neo_game_fix = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
		fix_bank=0;
	} };
			...
			if (flipx != 0)	/* X flip */
			{
				...
				else
				{
					for (y = sy;y <= ey;y++)
					{
						...
						mydword = fspr[1];
						if (dda_x_skip[ 0]) { col = (mydword>> 0)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
						if (dda_x_skip[ 1]) { col = (mydword>> 4)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
						if (dda_x_skip[ 2]) { col = (mydword>> 8)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
						if (dda_x_skip[ 3]) { col = (mydword>>12)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
						if (dda_x_skip[ 4]) { col = (mydword>>16)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
						if (dda_x_skip[ 5]) { col = (mydword>>20)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
						if (dda_x_skip[ 6]) { col = (mydword>>24)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
						if (dda_x_skip[ 7]) { col = (mydword>>28)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
	
						mydword = fspr[0];
						if (dda_x_skip[ 8]) { col = (mydword>> 0)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
						if (dda_x_skip[ 9]) { col = (mydword>> 4)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
						if (dda_x_skip[10]) { col = (mydword>> 8)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
						if (dda_x_skip[11]) { col = (mydword>>12)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
						if (dda_x_skip[12]) { col = (mydword>>16)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
						if (dda_x_skip[13]) { col = (mydword>>20)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
						if (dda_x_skip[14]) { col = (mydword>>24)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
						if (dda_x_skip[15]) { col = (mydword>>28)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
			...
			else		/* normal */
			{
				...
				else
				{
					for (y = sy ;y <= ey;y++)
					{
						...
						mydword = fspr[0];
						if (dda_x_skip[ 0]) { col = (mydword>>28)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
						if (dda_x_skip[ 1]) { col = (mydword>>24)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
						if (dda_x_skip[ 2]) { col = (mydword>>20)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
						if (dda_x_skip[ 3]) { col = (mydword>>16)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
						if (dda_x_skip[ 4]) { col = (mydword>>12)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
						if (dda_x_skip[ 5]) { col = (mydword>> 8)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
						if (dda_x_skip[ 6]) { col = (mydword>> 4)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
						if (dda_x_skip[ 7]) { col = (mydword>> 0)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
	
						mydword = fspr[1];
						if (dda_x_skip[ 8]) { col = (mydword>>28)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
						if (dda_x_skip[ 9]) { col = (mydword>>24)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
						if (dda_x_skip[10]) { col = (mydword>>20)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
						if (dda_x_skip[11]) { col = (mydword>>16)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
						if (dda_x_skip[12]) { col = (mydword>>12)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
						if (dda_x_skip[13]) { col = (mydword>> 8)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
						if (dda_x_skip[14]) { col = (mydword>> 4)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
						if (dda_x_skip[15]) { col = (mydword>> 0)&0xf; if (col != 0) *bm = paldata[col]; bm++; }
	...
	*/
	...
	void neogeo_vh_raster_partial_refresh(struct osd_bitmap *bitmap,int current_line)
	{
		...
		{
	//if (errorlog != 0) fprintf(errorlog,"refresh %d-%d\n",clip.min_y,clip.max_y);
