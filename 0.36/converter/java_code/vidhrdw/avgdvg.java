...
public class avgdvg
{
	...
	INLINE public static WriteHandlerPtr vector_timer = new WriteHandlerPtr() { public void handler(int deltax, int deltay)
	{
		deltax = abs (deltax);
		deltay = abs (deltay);
	...
	static void dvg_generate_vector_list(void)
	{
		...
		while (!done)
		{
			...
			if (vg_step != 0)
			{
		  		if (errorlog != 0) fprintf (errorlog,"Current beam position: (%d, %d)\n",
					currentx, currenty);
		  		getchar();
			}
	#endif
			...
			opcode = firstwd >> 12;
	#ifdef VG_DEBUG
			if (errorlog != 0) fprintf (errorlog,"%4x: %4x ", map_addr (pc), firstwd);
	#endif
			pc++;
	...
	#ifdef VG_DEBUG
			else if (errorlog != 0) fprintf (errorlog,"Illegal opcode ");
	#endif
			...
			switch (opcode)
			{
				case 0:
	#ifdef VG_DEBUG
		 			if (errorlog != 0) fprintf (errorlog,"Error: DVG opcode 0!  Addr %4x Instr %4x %4x\n", map_addr (pc-2), firstwd, secondwd);
					done = 1;
					break;
	#endif
				case 1:
				...
				case 9:
					...
					z = secondwd >> 12;
	#ifdef VG_DEBUG
					if (errorlog != 0) fprintf (errorlog,"(%d,%d) z: %d scal: %d", x, y, z, opcode);
	#endif
	...
	#ifdef VG_DEBUG
					if (errorlog != 0) fprintf (errorlog,"(%d,%d) scal: %d", x, y, secondwd >> 12);
	#endif
					break;
				...
				case DJSRL:
					a = firstwd & 0x0fff;
	#ifdef VG_DEBUG
					if (errorlog != 0) fprintf (errorlog,"%4x", map_addr(a));
	#endif
					stack [sp] = pc;
					if (sp == (MAXSTACK - 1))
		    			{
				...
				case DSVEC:
					...
					if (temp > 9)
						temp = -1;
	#ifdef VG_DEBUG
					if (errorlog != 0) fprintf (errorlog,"(%d,%d) z: %d scal: %d", x, y, z, temp);
	#endif
				...
				default:
					if (errorlog != 0) fprintf (errorlog,"Unknown DVG opcode found\n");
					done = 1;
			}
	#ifdef VG_DEBUG
	      		if (errorlog != 0) fprintf (errorlog,"\n");
	...
	*/
	...
	static void avg_generate_vector_list (void)
	{
		...
		{
			if (errorlog != 0) fprintf (errorlog,"VGO with zeroed vector memory\n");
			return;
		}
		...
		while (!done)
		{
			...
			if (opcode == VCTR)
			{
				...
				pc++;
	#ifdef VG_DEBUG
				if (errorlog != 0) fprintf (errorlog,"%4x  ", secondwd);
	#endif
			}
	#ifdef VG_DEBUG
			else if (errorlog != 0) fprintf (errorlog,"      ");
	#endif
	...
	#ifdef VG_DEBUG
			if (errorlog != 0) fprintf (errorlog,"%s ", avg_mnem [opcode]);
	#endif
			...
			switch (opcode)
			{
	...
	#ifdef VG_DEBUG
					if (errorlog != 0) fprintf (errorlog,"VCTR x:%d y:%d z:%d statz:%d", x, y, z, statz);
	#endif
					break;
	...
	#ifdef VG_DEBUG
					if (errorlog != 0) fprintf (errorlog,"SVEC x:%d y:%d z:%d statz:%d", x, y, z, statz);
	#endif
					break;
				...
				default:
					if (errorlog != 0) fprintf (errorlog,"internal error\n");
			}
	#ifdef VG_DEBUG
			if (errorlog != 0) fprintf (errorlog,"\n");
	#endif
	...
	public static WriteHandlerPtr avgdvg_go = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
	...
	public static WriteHandlerPtr avgdvg_reset = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
		avgdvg_clr_busy (0);
	} };
	
	public static ReadHandlerPtr avgdvg_init = new ReadHandlerPtr() { public int handler(int vgType)
	{
		int i;
	...
	static public static WriteHandlerPtr colorram_w = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
	...
	public static WriteHandlerPtr quantum_colorram_w = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
	/* Notes on colors:
	...
	***************************************************************************/
	...
	void avg_stop(void)
	{
		...
		if (backdrop != 0) artwork_free(backdrop);
		backdrop = NULL;
	}
