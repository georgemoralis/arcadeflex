...
public class m107
{
	...
	static public static WriteHandlerPtr get_pf4_tile_info = new WriteHandlerPtr() { public void handler(int col, int row)
	{
		int offs,tile,color;
		offs=(col*4) + (row*256);
		offs+=pf4_vram_ptr;
	...
	{
		if (offset==0) {
	//		if (errorlog != 0) fprintf(errorlog,"%04x: buffered spriteram\n",cpu_get_pc());
