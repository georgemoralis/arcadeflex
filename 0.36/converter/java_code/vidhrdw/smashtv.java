...
public class smashtv
{
	...
	public static VhStartPtr wms_vh_start = new VhStartPtr() { public int handler() 
	{
		if ((wms_cmos_ram = malloc(0x8000)) == 0)
		{
			if (errorlog != 0) fprintf(errorlog, "smashtv.c: Couldn't Alloc CMOS RAM\n");
			return 1;
		}
		if ((paletteram = malloc(0x4000)) == 0)
		{
			if (errorlog != 0) fprintf(errorlog, "smashtv.c: Couldn't Alloc color RAM\n");
			free(wms_cmos_ram);
			return 1;
		}
		if ((wms_videoram = malloc(wms_videoram_size)) == 0)
		{
			if (errorlog != 0) fprintf(errorlog, "smashtv.c: Couldn't Alloc video RAM\n");
			free(wms_cmos_ram);
			free(paletteram);
			return 1;
		}
	...
	public static VhStartPtr wms_t_vh_start = new VhStartPtr() { public int handler() 
	{
	...
	public static VhStopPtr wms_vh_stop = new VhStopPtr() { public void handler() 
	{
		free(wms_cmos_ram);
		free(wms_videoram);
		free(paletteram);
	...
	void wms_vh_eof(void)
	{
		...
		if (wms_autoerase_reset != 0)
			wms_autoerase_enable = 0;
	}
	
	public static VhUpdatePtr wms_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
