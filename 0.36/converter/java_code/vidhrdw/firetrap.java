...
public class firetrap
{
	...
	***************************************************************************/
	public static VhConvertColorPromPtr firetrap_vh_convert_color_prom = new VhConvertColorPromPtr() { public void handler(UByte []palette, char []colortable, UBytePtr color_prom) 
	{
		int i;
	...
	***************************************************************************/
	public static VhStopPtr firetrap_vh_stop = new VhStopPtr() { public void handler() 
	{
		osd_free_bitmap(tmpbitmap);
		...
		for (offs = firetrap_bgvideoram_size - 1;offs >= 0;offs--)
		{
			if ((offs & 0x100) == 0)
			{
				if (dirtybuffer[offs] || dirtybuffer[offs+0x100])
				{
					...
					sx = (offs / 16) & 0x0f;
					sy = 31 - offs % 16;
					if ((offs & 0x200) != 0) sy -= 16;
					if ((offs & 0x400) != 0) sx += 16;
