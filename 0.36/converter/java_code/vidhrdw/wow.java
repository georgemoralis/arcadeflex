...
public class wow
{
	...
	public static WriteHandlerPtr wow_update_line = new WriteHandlerPtr() { public void handler(int line, int gorf);
	...
	#ifdef MAME_DEBUG
	    if (errorlog != 0) fprintf(errorlog,"colors split set to %02d\n",ColourSplit);
	#endif
	#endif
	} };
	...
	#ifdef MAME_DEBUG
	    if (errorlog != 0) fprintf(errorlog,"colors %01x set to %02x\n",offset,data);
	#endif
	} };
	...
	{
	#ifdef MAME_DEBUG
		if (errorlog != 0) fprintf(errorlog,"%04x: magic_expand_color = %02x\n",cpu_get_pc(),data);
	#endif
	...
	{
	#ifdef MAME_DEBUG
		if (errorlog != 0) fprintf(errorlog,"%04x: magic_control = %02x\n",cpu_get_pc(),data);
	#endif
	...
	static public static WriteHandlerPtr copywithflip = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
		...
		if ((magic_control & 0x40) != 0)	/* copy backwards */
		{
			...
			if ((magic_control & 0x20) != 0) data1 ^= wow_videoram[offset-1];	/* draw in XOR mode */
			else if ((magic_control & 0x10) != 0) data1 |= wow_videoram[offset-1];	/* draw in OR mode */
	...
	#ifdef MAME_DEBUG
			if (errorlog != 0) fprintf(errorlog,"%04x: blit src %04x mode %02x skip %d dest %04x length %d loops %d\n",
	...
	#ifdef MAME_DEBUG
	    if (errorlog != 0) fprintf(errorlog,"Gorf Latch IO %02x set to %d (%02x)\n",(offset << 3) + (data >> 1),data & 0x01,data);
	#endif
	...
	public static VhUpdatePtr gorf_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
		int offs;
	    int Sparkle=0;
	    int SparkleLow=0;
	...
	public static VhStartPtr wow_vh_start = new VhStartPtr() { public int handler() 
	{
	...
	public static VhUpdatePtr wow_vh_screenrefresh_stars = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
	  	copybitmap(bitmap,tmpbitmap,0,0,0,0,&Machine.drv.visible_area,TRANSPARENCY_NONE,0);
	...
	public static VhUpdatePtr wow_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
		copybitmap(bitmap,tmpbitmap,0,0,0,0,&Machine.drv.visible_area,TRANSPARENCY_NONE,0);
	} };
