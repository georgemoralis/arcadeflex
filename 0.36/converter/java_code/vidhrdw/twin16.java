...
public class twin16
{
	...
	public static WriteHandlerPtr twin16_video_register_w = new WriteHandlerPtr() { public void handler(int offset, int data){
		switch( offset ){
			...
			case 0xe:
			if (errorlog != 0) fprintf( errorlog, "unknown video_register write:%d", data );
			break;
		}
	} };
	...
	static void draw_text( struct osd_bitmap *bitmap ){
		...
		if ((video_register & TWIN16_SCREEN_FLIPX) != 0) tile_flipx = !tile_flipx;
						...
						else
						{
							if (bitmap.depth == 16)
							{
								for( y=y1; y!=y2; y+=yd )
								{
									...
									if (data != 0)
									{
										pen = (data>>4*3)&0xf; if (pen != 0) dest[7] = pal_data[pen];
										pen = (data>>4*2)&0xf; if (pen != 0) dest[6] = pal_data[pen];
										pen = (data>>4*1)&0xf; if (pen != 0) dest[5] = pal_data[pen];
										pen = (data>>4*0)&0xf; if (pen != 0) dest[4] = pal_data[pen];
									}
									data = *gfx_data++;
									if (data != 0)
									{
										pen = (data>>4*3)&0xf; if (pen != 0) dest[3] = pal_data[pen];
										pen = (data>>4*2)&0xf; if (pen != 0) dest[2] = pal_data[pen];
										pen = (data>>4*1)&0xf; if (pen != 0) dest[1] = pal_data[pen];
										pen = (data>>4*0)&0xf; if (pen != 0) dest[0] = pal_data[pen];
									}
								}
							}
							else
						...
						else
						{
							if (bitmap.depth == 16)
							{
								for( y=y1; y!=y2; y+=yd )
								{
									...
									if (data != 0)
									{
										pen = (data>>4*3)&0xf; if (pen != 0) dest[0] = pal_data[pen];
										pen = (data>>4*2)&0xf; if (pen != 0) dest[1] = pal_data[pen];
										pen = (data>>4*1)&0xf; if (pen != 0) dest[2] = pal_data[pen];
										pen = (data>>4*0)&0xf; if (pen != 0) dest[3] = pal_data[pen];
									}
									data = *gfx_data++;
									if (data != 0)
									{
										pen = (data>>4*3)&0xf; if (pen != 0) dest[4] = pal_data[pen];
										pen = (data>>4*2)&0xf; if (pen != 0) dest[5] = pal_data[pen];
										pen = (data>>4*1)&0xf; if (pen != 0) dest[6] = pal_data[pen];
										pen = (data>>4*0)&0xf; if (pen != 0) dest[7] = pal_data[pen];
									}
								}
							}
							else
