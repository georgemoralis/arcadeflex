...
public class gameplan
{
	...
	***************************************************************************/
	...
	{
	#ifdef VERBOSE
		if (errorlog != 0)  fprintf(errorlog, "GAME:  read reg%X at PC %04x\n", offset, cpu_get_pc());
	#endif
		...
		if (offset == 0)
		{
			...
			if (errorlog != 0)
			{
				if (finished_sound != 0)  fprintf(errorlog, "[GAME: checking sound request ack: OK (%d)]\n", finished_sound);
	...
	{
	#ifdef VERBOSE
		if (errorlog != 0)  fprintf(errorlog, "GAME: write reg%X with %02x at PC %04x\n", offset, data, cpu_get_pc());
	#endif
	
		if (offset == 1)
		{
	#ifdef VERBOSE
			if (errorlog != 0) fprintf(errorlog, "[GAME: request sound number %d]\n", data);
	#endif
	...
	{
	#ifdef VERBOSE
		if (errorlog != 0)  fprintf(errorlog, "SOUND:  read reg%X at PC %04x\n", offset, cpu_get_pc());
	#endif
		...
		if (offset == 0)
		{
			new_request = 0;
	#ifdef VERBOSE
			if (errorlog != 0) fprintf(errorlog, "[SOUND: received sound request %d]\n", port_b);
	#endif
			return port_b;
		}
		...
		if (offset == 5)
		{
			...
			{
	#ifdef VERBOSE
				if (errorlog != 0) fprintf(errorlog, "[SOUND: checking for new request - found]\n");
	#endif
				return 0x40;
			}
			else
			{
	#ifdef VERBOSE
				if (errorlog != 0) fprintf(errorlog, "[SOUND: checking for new request - none]\n");
	#endif
				return 0;
			}
		}
	...
	{
	#ifdef VERBOSE
		if (errorlog != 0)  fprintf(errorlog, "SOUND: write reg%X with %02x at PC %04x\n", offset, data, cpu_get_pc());
	#endif
	
		if (offset == 2)
		{
	#ifdef VERBOSE
			if (errorlog != 0) fprintf(errorlog, "[SOUND: ack received request %d]\n", data);
	#endif
			finished_sound = data;
		}
	} };
	...
	public static ReadHandlerPtr gameplan_video_r = new ReadHandlerPtr() { public int handler(int offset)
	{
		...
		x++;
	#if 0
		if (errorlog != 0) fprintf(errorlog, "%04x: reading %d from 200d\n", cpu_get_pc(), x);
	#endif
		return x;
	} };
	...
	public static WriteHandlerPtr gameplan_video_w = new WriteHandlerPtr() { public void handler(int offset, int data)
	{
	...
	#ifdef VERBOSE
		if (errorlog != 0)  fprintf(errorlog, "VIA 1: PC %04x: %x . reg%X\n", cpu_get_pc(), data, offset);
	#endif
		...
		if (offset == 0)			/* write to 2000 */
		{
			r0 = data;
	#ifdef VERBOSE
			if (errorlog != 0) fprintf(errorlog, "  mode = %d\n", data);
	#endif
		}
		else if (offset == 1)		/* write to 2001 */
		{
			if (r0 == 0)
			{
				...
				{
	#ifdef VERBOSE
					if (errorlog != 0) fprintf(errorlog, "  !movement command %02x unknown\n", data);
	#endif
				}
	...
	#ifdef VERBOSE_VIDEO
	#ifndef SHOW_CHARS
				if (errorlog != 0) fprintf(errorlog, "  Y = %d\n", ypos);
	#endif
	#endif
			}
			else if (r0 == 3)
			{
				...
				{
	#ifdef VERBOSE_VIDEO
					if (errorlog != 0) fprintf(errorlog, "  clear screen\n");
	#endif
					gameplan_clear_screen();
				}
	#ifdef VERBOSE
				else if (errorlog != 0)  fprintf(errorlog, "  !not clear screen: offset = %d, data = %d\n", offset, data);
	#endif
			}
	#ifdef VERBOSE
			else
			{
				if (errorlog != 0)  fprintf(errorlog, "  !offset = %d, data = %02x\n", offset, data);
			}
	#endif
		}
		else if (offset == 2)
	...
	#ifdef VERBOSE_VIDEO
	#ifndef SHOW_CHARS
				if (errorlog != 0)  fprintf(errorlog, "  colour %d, move to (%d, %d)\n", colour, xpos, ypos);
	#endif
	#endif
			}
	...
	#ifdef VERBOSE
				if (errorlog != 0) fprintf(errorlog, "  unusual colour request %d\n", data & 7);
	#endif
			}
	#ifdef VERBOSE
			else
	...
	***************************************************************************/
	...
	{
	#ifdef VERBOSE_VIDEO
		if (errorlog != 0)  fprintf(errorlog, "  clearing the screen to colour %d (%s)\n", clear_to_colour, colour_names[clear_to_colour]);
	#endif
