...
public class crtc6845
{
	...
	public static ReadHandlerPtr crtc6845_register_r = new ReadHandlerPtr() { public int handler(int address)
	{
		int retval=0;
	...
	public static WriteHandlerPtr crtc6845_address_w = new WriteHandlerPtr() { public void handler(int address, int data)
	{
	        crtc6845_address_latch=data&0x1f;
	} };
	
	
	public static WriteHandlerPtr crtc6845_register_w = new WriteHandlerPtr() { public void handler(int address, int data)
	{
	
	if (errorlog != 0) fprintf(errorlog,"CRT #0 PC %04x: WRITE reg 0x%02x data 0x%02x\n",cpu_get_pc(),crtc6845_address_latch,data);
