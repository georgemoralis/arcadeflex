...
public class dday
{
			...
			case TRANSPARENCY_PEN:
				...
				for (y = sy;y <= ey;y++)
				{
					...
					for( bm += sx ; bm <= bme-3 ; bm+=4, f^=1 )
					{
						...
						{
							col = col4;
							if (col != 0)  bm[BL0] = paldata[col+((shadow & 0x01) << 8)];
							col = col4>>8;
							if (col != 0)  bm[BL1] = paldata[col+((shadow & 0x02) << 7)];
							col = col4>>16;
							if (col != 0)  bm[BL2] = paldata[col+((shadow & 0x04) << 6)];
							col = col4>>24;
							if (col != 0)  bm[BL3] = paldata[col+((shadow & 0x08) << 5)];
						}
						sd4++;
					}
					start+=1;
	...
	***************************************************************************/
	public static VhConvertColorPromPtr dday_vh_convert_color_prom = new VhConvertColorPromPtr() { public void handler(UByte []palette, char []colortable, UBytePtr color_prom) 
	{
		int i,total;
	...
	***************************************************************************/
	public static VhUpdatePtr dday_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
		int offs;
