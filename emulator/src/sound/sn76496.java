...
public class sn76496 extends sndintrf.snd_interface
{
        ...
        while (length > 0)
	{
		...
		if (out > MAX_OUTPUT * STEP) out = MAX_OUTPUT * STEP;
                ...
                buffer.writeinc((int)(out/STEP));//*(buffer++) = out / STEP;
    ...
    static void SN76496_set_clock(int chip,int clock)
    {
            ...
            /* number. */         
            R.UpdateStep = (int)((STEP * R.SampleRate * 16) /clock);//R->UpdateStep = ((double)STEP * R->SampleRate * 16) / clock;
    ...
    static void SN76496_set_gain(int chip,int gain)
    {
            ...
            while (gain-- > 0)
                    out = (double)(out* 1.023292992);	/* = (10 ^ (0.2/20)) */

            /* build volume table (2dB per step) */
            for (i = 0;i < 15;i++)
            {
                    /* limit volume to avoid clipping */
                    if (out > MAX_OUTPUT / 3) R.VolTable[i] = (int)(MAX_OUTPUT / 3);
