    ...
    ******************************************************************************/
    ...
    public static void fdwarrio_decode()
    {
    	...
    	for (A = 0x0000;A < 0x8000;A++)
    	{
    		...
    		/* the bottom half of the translation table is the mirror image of the top */
    		if ((src & 0x40)!=0) col = 7 - col;
    
    		/* decode the opcodes */
    		rom.write(A + diff,src ^ xortable[row >> 3][col] ^ 0x40);
    		if ((row & 1)!=0) rom.write(A + diff,rom.read(A + diff) ^ 0x10);
    		if ((row & 2)!=0) rom.write(A + diff,rom.read(A + diff) ^ 0x04);
    		if ((row & 4)!=0) rom.write(A + diff,rom.read(A + diff) ^ 0x01);
    
    		/* decode the data */
    		row++;	/* the data XOR table is shifted by one position!!!! */
    		rom.write(A,src ^ xortable[row >> 3][col]);
    		if ((row & 1)!=0) rom.write(A, rom.read(A) ^ 0x10);
    		if ((row & 2)!=0) rom.write(A, rom.read(A) ^ 0x04);
    		if ((row & 4)!=0) rom.write(A, rom.read(A) ^ 0x01);
    	}
