...
public class victory
{
	...
	static class _micro
	{
        ...
        static _micro micro = new _micro();
	...
	/* if it's not 8 */
	public static int MICRO_STATE_CLOCK_PERIOD	= (int) TIME_IN_HZ(11827000/8);
	...
	*/
	...
	public static VhEofCallbackPtr victory_vh_eof = new VhEofCallbackPtr() {
		...
		for (y = 0; y < 256; y++)
		{
			...
			{
				UShortPtr fg = new UShortPtr(fgbitmap.line[sy], 0);
				UShortPtr bg = new UShortPtr(bgbitmap.line[y], 0);
	...
	public static VhUpdatePtr victory_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
		...
		for (y = 0; y < 256; y++)
		{
			...
			{
				UShortPtr fg = new UShortPtr(fgbitmap.line[y], 0);
				UShortPtr bg = new UShortPtr(bgbitmap.line[sy], 0);
				UShortPtr dst = new UShortPtr(bitmap.line[y], 0);
	
				for (x = 0; x < 256; x++)
				{
					int fpix = fg.read(0);fg.inc(1);
					int bpix = bg.read((x + scrollx) & 255);
