...
public class ataripf {
 ...
 /* internal state structure containing values that can change scanline-by-scanline */
    ...
    public static class ataripf_data {
        ...
        int timerallocated;
        /* true if we've allocated the timer */
        int gfxchanged;
        /* true if the gfx info has changed */

        int colshift;
        /* bits to shift X coordinate when looking up in VRAM */
        int rowshift;
        /* bits to shift Y coordinate when looking up in VRAM */
        int colmask;
        /* mask to use when wrapping X coordinate in VRAM */
        int rowmask;
        /* mask to use when wrapping Y coordinate in VRAM */
        int vrammask;
        /* combined mask when accessing VRAM with raw addresses */
        int vramsize;
        /* total size of VRAM, in entries */

        int tilexshift;
        /* bits to shift X coordinate when drawing */
        int tileyshift;
        /* bits to shift Y coordinate when drawing */
        int tilewidth;
        /* width of a single tile */
        int tileheight;
        /* height of a single tile */
        int bitmapwidth;
        /* width of the full playfield bitmap */
        int bitmapheight;
        /* height of the full playfield bitmap */
        int bitmapxmask;
        /* x coordinate mask for the playfield bitmap */
        int bitmapymask;
        /* y coordinate mask for the playfield bitmap */
        ...
        int shadowxor;
        /* color XOR for shadow effect (if any) */
        int transpens;
        /* transparent pen */
        int transpen;
        /* transparent pen */
        ...
        int lookupmask;
        /* mask for the lookup table */

        int latchval;
        /* value for latching */
        int latchdata;
        /* shifted value for latching */
        int latchmask;
        /* mask for latching */
        ...
        ataripf_state curstate;
        /* current state */
        ataripf_state[] statelist;
        /* list of changed states */
        int _ataripf_state = 0;
        int stateindex;
        /* index of the next state */

        rectangle process_clip = new rectangle();
        /* (during processing) the clip rectangle */
        rectangle process_tiles = new rectangle();
        /* (during processing) the tiles rectangle */
        Object process_param;
        /* (during processing) the callback parameter */
 /*TODO*///	
        ataripf_gfxelement[] gfxelement = new ataripf_gfxelement[MAX_GFX_ELEMENTS];
        /* graphics element copies */
        int max_usage_words;
        /* maximum words of usage */
    };
 ...
 /*---------------------------------------------------------------
    ...
    public static void pf_update_state(ataripf_data pf, int scanline) {
        ...
        if (pf.statelist[_idx - 1].scanline > scanline) {
            ...
            return;
        } /* if this is the same scanline as last time, overwrite it */ else if (pf.statelist[_idx - 1].scanline == scanline) {
            logerror("pf_update_state: scanlines equal, overwriting\n");
            //state--;
            _idx--;
        } /* otherwise, move forward one entry */ else {
...
/*TODO*///	}
	...
	---------------------------------------------------------------*/
    static TimerCallbackHandlerPtr pf_eof_callback = new TimerCallbackHandlerPtr() {
        @Override
