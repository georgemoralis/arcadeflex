...
public class lwings
{
	...
	*/
	...
	public static VhStopPtr trojan_vh_stop = new VhStopPtr() { public void handler() 
	{
		...
		for( offs = spriteram_size[0] - 4;offs >= 0;offs -= 4 ){
			...
			*/
			...
			if( sx!=0 && sy!=0 ){
				...
				if ((attrib & 0x40) != 0) code += 256;
				if ((attrib & 0x80) != 0) code += 256*4;
				if ((attrib & 0x20) != 0) code += 256*2;
