...
public class cps1draw {
	...
	if ((Machine.orientation & ORIENTATION_SWAP_XY)!=0)
	{
		...
		for (i=0; i<size; i++)
		{
			...
			for (j=0; j<size/8; j++)
			{
				dwval=Math.abs(cps1_gfx[src_ptr]);
				n=(dwval>>28)&0x0f;
	...
	else
	{
		...
		if (flipx != 0)
		{
			...
			for (i=0; i<size; i++)
			{
			/*	if (flipy != 0) bm=(DATATYPE *)dest.line[sy-i]+sx;
				else bm=(DATATYPE *)dest.line[sy+i]+sx;
				for (j=0; j<size/8; j++)
				{
					dwval=*src;
					n=(dwval>>28)&0x0f;
					if ((tpens & (0x01 << n))!=0) bm[-1]=paldata[n];
					n=(dwval>>24)&0x0f;
					if ((tpens & (0x01 << n))!=0) bm[-2]=paldata[n];
					n=(dwval>>20)&0x0f;
					if ((tpens & (0x01 << n))!=0) bm[-3]=paldata[n];
					n=(dwval>>16)&0x0f;
					if ((tpens & (0x01 << n))!=0) bm[-4]=paldata[n];
					n=(dwval>>12)&0x0f;
					if ((tpens & (0x01 << n))!=0) bm[-5]=paldata[n];
					n=(dwval>>8)&0x0f;
					if ((tpens & (0x01 << n))!=0) bm[-6]=paldata[n];
					n=(dwval>>4)&0x0f;
					if ((tpens & (0x01 << n))!=0) bm[-7]=paldata[n];
					n=dwval&0x0f;
					if ((tpens & (0x01 << n))!=0) bm[-8]=paldata[n];
					bm-=8;
					src++;
				}
				src+=srcdelta;
