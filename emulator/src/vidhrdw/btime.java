...
public class btime
{
	...
	public static CharPtr lnc_charbank=new CharPtr();
	public static CharPtr bnj_backgroundram=new CharPtr();
	public static CharPtr zoar_scrollram=new CharPtr();
	public static CharPtr deco_charram=new CharPtr();
	...
	***************************************************************************/
	public static VhConvertColorPromPtr btime_vh_convert_color_prom = new VhConvertColorPromPtr() { public void handler(UByte []palette, char []colortable, UBytePtr color_prom) 
	{
	    int i;
	...
	***************************************************************************/
	public static VhConvertColorPromPtr lnc_vh_convert_color_prom = new VhConvertColorPromPtr() { public void handler(UByte []palette, char []colortable, UBytePtr color_prom) 
	{
	    int i;
	...
	***************************************************************************/
	...
	static void drawsprites(osd_bitmap bitmap, int color,
	                        ...
	                        CharPtr sprite_ram, int interleave)
	{
	    int i,offs;
	...
	static void drawbackground(osd_bitmap bitmap, CharPtr tilemap)
	{
	    int i, offs;
	...
	static void decode_modified(CharPtr sprite_ram, int interleave)
	{
	    int i,offs;
	    ...
	    for (offs = videoram_size[0] - 1;offs >= 0;offs--)
	    {
	        ...
	        {
	        case 1:
	            decodechar(Machine.gfx[0],code,new UBytePtr(deco_charram.memory,deco_charram.base),Machine.drv.gfxdecodeinfo[0].gfxlayout);
	            char_dirty[code] = 2;
	            /* fall through */
	        case 2:
	            dirtybuffer[offs] = 1;
	    ...
	    for (i = 0, offs = 0;i < 8; i++, offs += 4*interleave)
	    {
	        ...
	        if (sprite_dirty[code]!=0)
	        {
	            ...
	            decodechar(Machine.gfx[1],code,new UBytePtr(deco_charram.memory,deco_charram.base),Machine.drv.gfxdecodeinfo[1].gfxlayout);
	        }
	    }
	}
	...
	public static VhUpdatePtr btime_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
	    ...
	    if ((bnj_scroll1 & 0x10) != 0)
	    {
	        ...
	        drawbackground(bitmap, new CharPtr(btime_tilemap));
	...
	public static VhUpdatePtr zoar_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
	    ...
	    drawsprites(bitmap, btime_palette + 1, 1, 2, new CharPtr(videoram,0x1f), 0x20);
