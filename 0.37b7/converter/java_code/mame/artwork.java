...
public class artwork
{
	...
	*********************************************************************/
	...
	{
		if (artwork_backdrop != 0) backdrop_remap();
		if (artwork_overlay != 0) overlay_remap();
	}
	...
	void artwork_draw(struct osd_bitmap *dest, struct osd_bitmap *source, int _bitmap_dirty)
	{
		...
		if (artwork_backdrop != 0) backdrop_draw(dest, source);
		if (artwork_overlay != 0) overlay_draw(dest, source);
	}
	...
	void artwork_kill (void)
	{
		...
		if (artwork_backdrop != 0) artwork_free(&artwork_backdrop);
		if (artwork_overlay != 0) artwork_free(&artwork_overlay);
	}
