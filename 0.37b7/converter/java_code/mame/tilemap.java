...
public class tilemap
{
	...
	/* TBA: combine with memcpybitmask */
		for(;;){
			UINT32 data = *bitmask++;
			if ((data & 0x80) != 0) dest[0] |= value;
			if ((data & 0x40) != 0) dest[1] |= value;
			if ((data & 0x20) != 0) dest[2] |= value;
			if ((data & 0x10) != 0) dest[3] |= value;
			if ((data & 0x08) != 0) dest[4] |= value;
			if ((data & 0x04) != 0) dest[5] |= value;
			if ((data & 0x02) != 0) dest[6] |= value;
			if ((data & 0x01) != 0) dest[7] |= value;
			if( --count == 0 ) break;
			dest+=8;
		}
	}
