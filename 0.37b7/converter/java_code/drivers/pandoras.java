...
public class pandoras
{
	...
	public static WriteHandlerPtr pandoras_int_control_w = new WriteHandlerPtr() {public void handler(int offset, int data){
		...
		switch (offset){
			case 0x00:	if (!data) cpu_set_irq_line(0, M6809_IRQ_LINE, CLEAR_LINE);
						irq_enable_a = data;
						break;
			case 0x02:	coin_counter_w.handler(0,data & 0x01);
						break;
			case 0x03:	coin_counter_w.handler(1,data & 0x01);
						break;
	...
	***************************************************************************/
	...
	//	PORT_DIPSETTING(    0x00, "Invalid" );
		...
		PORT_DIPSETTING(    0x18, "20k and every 60k" );	PORT_DIPSETTING(    0x10, "30k and every 70k" );	PORT_DIPSETTING(    0x08, "20k" );	PORT_DIPSETTING(    0x00, "30k" );	PORT_DIPNAME( 0x60, 0x60, DEF_STR( "Difficulty") );
		...
		PORT_START(); 	/* COINSW */
		PORT_BIT( 0x01, IP_ACTIVE_LOW, IPT_COIN1 );	PORT_BIT( 0x02, IP_ACTIVE_LOW, IPT_COIN2 );	PORT_BIT( 0x04, IP_ACTIVE_LOW, IPT_COIN3 );	PORT_BIT( 0x08, IP_ACTIVE_LOW, IPT_START1 );	PORT_BIT( 0x10, IP_ACTIVE_LOW, IPT_START2 );	PORT_BIT( 0x20, IP_ACTIVE_LOW, IPT_UNKNOWN );	PORT_BIT( 0xc0, IP_ACTIVE_LOW, IPT_UNUSED );
	...
	***************************************************************************/
	...
	static RomLoadPtr rom_pandoras = new RomLoadPtr(){ public void handler(){ 
		...
		ROM_LOAD( "pand_j13.cpu",	0x08000, 0x02000, 0x7a0fe9c5 );	ROM_LOAD( "pand_j12.cpu",	0x0a000, 0x02000, 0x7dc4bfe1 );	ROM_LOAD( "pand_j10.cpu",	0x0c000, 0x02000, 0xbe3af3b7 );	ROM_LOAD( "pand_j9.cpu",	0x0e000, 0x02000, 0xe674a17a );
		...
		ROM_REGION( 0x4000, REGION_GFX1 | REGIONFLAG_DISPOSE );	ROM_LOAD( "pand_a18.cpu",	0x00000, 0x02000, 0x23706d4a );/* tiles */
		ROM_LOAD( "pand_a19.cpu",	0x02000, 0x02000, 0xa463b3f9 );
		ROM_REGION( 0x6000, REGION_GFX2 | REGIONFLAG_DISPOSE );	ROM_LOAD( "pand_j18.cpu",	0x00000, 0x02000, 0x99a696c5 );/* sprites */
		ROM_LOAD( "pand_j17.cpu",	0x02000, 0x02000, 0x38a03c21 );	ROM_LOAD( "pand_j16.cpu",	0x04000, 0x02000, 0xe0708a78 );
		ROM_REGION( 0x0220, REGION_PROMS );	ROM_LOAD( "pandora.2a",		0x0000, 0x020, 0x4d56f939 );/* palette */
