...
public class powerins
{
	...
	***************************************************************************/
	...
	public static ReadHandlerPtr powerins_vregs_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		return READ_WORD(&powerins_vregs[offset]);
	} };
	
	public static WriteHandlerPtr powerins_vregs_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
	...
	***************************************************************************/
	...
	public static WriteHandlerPtr powerins_paletteram_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		/*	byte 0    byte 1	*/
		/*	RRRR GGGG BBBB RGBx	*/
		/*	4321 4321 4321 000x	*/
	...
	***************************************************************************/
	...
	static void powerins_draw_sprites(struct osd_bitmap *bitmap)
	{
		...
		for ( ; source < finish; source += 16 )
		{
			...
			if (flipscreen != 0)	{	sx = screen_w - sx - dimx*16 - 32;	flipx = !flipx;
	...
	***************************************************************************/
	...
	public static VhUpdatePtr powerins_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
		int layers_ctrl = -1;
	...
	//	if (keyboard_pressed(KEYCODE_E))	msk |= 4;
		...
		if ((layers_ctrl & 8) != 0)		powerins_draw_sprites(bitmap);
		if ((layers_ctrl & 2) != 0)		tilemap_draw(bitmap, tilemap_1, 0);
	} };
}
