...
public class williams
{
	...
	public static VhUpdatePtr williams_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
	...
	public static VhUpdatePtr blaster_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
		int j;
	...
	*/
	...
	/* define this so that we get the pixel copying code when we #define WILLIAMS_COPIES 				1
			...
			for (y = clip.min_y; y <= clip.max_y; y++)
			{
				...
				if (!(Machine.orientation & ORIENTATION_FLIP_X))
				{
					...
					for (x = 0; x < pairs; x++, source += 256, dest += 2)
					{
						...
						if (pix != 0)
						{
							...
							if (p1 != 0) dest[0] = pens[p1];
							if (p2 != 0) dest[1] = pens[p2];
						}
					}
				}
				...
				else
				{
					...
					for (x = 0; x < pairs; x++, source += 256, dest -= 2)
					{
						...
						{
							int p1 = pix >> 4, p2 = pix & 0x0f;
							if (p1 != 0) dest[-1] = pens[p1];
							if (p2 != 0) dest[-2] = pens[p2];
						}
					}
				}
			}
			...
			for (y = clip.min_y; y <= clip.max_y; y++)
			{
				...
				else
				{
					...
					for (x = 0; x < pairs; x++, source += 256, dest -= dy + dy)
					{
						...
						{
							int p1 = pix >> 4, p2 = pix & 0x0f;
							if (p1 != 0) dest[-dy] = pens[p1];
							if (p2 != 0) dest[-dy-dy] = pens[p2];
						}
					}
				}
			}
