...
public class arabian
{
	...
	public static VhConvertColorPromPtr arabian_vh_convert_color_prom = new VhConvertColorPromPtr() { public void handler(char []palette, char []colortable, UBytePtr color_prom) 
	{
		int i;
	...
	***************************************************************************/
	...
	public static WriteHandlerPtr arabian_videoram_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		...
		if (plane3 != 0)
		{
			...
			*bm &= 0xf3;
			if ((data & 0x10) != 0) *bm |= (16+8);
			if ((data & 0x01) != 0) *bm |= (16+4);
	
			bm = &tmpbitmap2.line[y+  dy][x+  dx];
			*bm &= 0xf3;
			if ((data & 0x20) != 0) *bm |= (16+8);
			if ((data & 0x02) != 0) *bm |= (16+4);
	
			bm = &tmpbitmap2.line[y+2*dy][x+2*dx];
			*bm &= 0xf3;
			if ((data & 0x40) != 0) *bm |= (16+8);
			if ((data & 0x04) != 0) *bm |= (16+4);
	
			bm = &tmpbitmap2.line[y+3*dy][x+3*dx];
			*bm &= 0xf3;
			if ((data & 0x80) != 0) *bm |= (16+8);
			if ((data & 0x08) != 0) *bm |= (16+4);
		}
		...
		if (plane4 != 0)
		{
			bm = &tmpbitmap2.line[y     ][x     ];
			*bm &= 0xfc;
			if ((data & 0x10) != 0) *bm |= (16+2);
			if ((data & 0x01) != 0) *bm |= (16+1);
	
			bm = &tmpbitmap2.line[y+  dy][x+  dx];
			*bm &= 0xfc;
			if ((data & 0x20) != 0) *bm |= (16+2);
			if ((data & 0x02) != 0) *bm |= (16+1);
	
			bm = &tmpbitmap2.line[y+2*dy][x+2*dx];
			*bm &= 0xfc;
			if ((data & 0x40) != 0) *bm |= (16+2);
			if ((data & 0x04) != 0) *bm |= (16+1);
	
			bm = &tmpbitmap2.line[y+3*dy][x+3*dx];
			*bm &= 0xfc;
			if ((data & 0x80) != 0) *bm |= (16+2);
			if ((data & 0x08) != 0) *bm |= (16+1);
		}
