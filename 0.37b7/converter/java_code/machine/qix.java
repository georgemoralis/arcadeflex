...
public class qix
{
	...
	***************************************************************************/
	...
	public static WriteHandlerPtr sdungeon_68705_portA_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
	...
	{
		portB_in = input_port_1_r.handler(0) & 0x0F;
		portB_in = portB_in | ((input_port_1_r.handler(0) & 0x80) >> 3);
	//logerror("PC: %x MCU PORTB R = %x\n",cpu_get_pc(),portB_in);
	
		return (portB_out & ddrB) | (portB_in & ~ddrB);
	} };
	
	public static WriteHandlerPtr sdungeon_68705_portB_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
	...
	{
		portC_in = (~sdungeon_coinctrl & 0x08) | ((input_port_1_r.handler(0) & 0x70) >> 4);
	//logerror("PC: %x MCU PORTC R = %x\n",cpu_get_pc(),portC_in);
	
		return (portC_out & ddrC) | (portC_in & ~ddrC);
	} };
	
	public static WriteHandlerPtr sdungeon_68705_portC_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
