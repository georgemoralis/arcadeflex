...
public class exidy
{
	...
	***************************************************************************/
	...
	public static ShStartPtr exidy_sh_start = new ShStartPtr() { public int handler(MachineSound msound) 
	{
		/* Init PIA */
	...
	***************************************************************************/
	...
	static void riot_interrupt(int parm)
	{
		if (riot_state == RIOT_COUNTUP)
		{
			...
			if (riot_irq_enable != 0) cpu_set_irq_line(1, M6502_INT_IRQ, ASSERT_LINE);
	...
	public static WriteHandlerPtr exidy_shriot_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		...
		{
			case 0:	/* port A */
				if (has_hc55516 != 0) cpu_set_reset_line(2, (data & 0x10) ? CLEAR_LINE : ASSERT_LINE);
			...
			case 0x1c:
				...
				riot_divider = 1;
				if (riot_timer != 0) timer_remove(riot_timer);
