...
public class fm
{
	...
	static void OPNWriteReg(FM_OPN *OPN, int r, int v)
	{
		...
		case 0x90:	/* SSG-EG */
	#if !FM_SEG_SUPPORT
			if ((v & 0x08) != 0) LOG(LOG_ERR,("OPN %d,%d,%d :SSG-TYPE envelope selected (not supported )\n",OPN.ST.index,c,OPN_SLOT(r)));
	#endif
			SLOT.SEG = v&0x0f;
			break;
		case 0xa0:
	...
	static void InitOPNB_ADPCMATable(void){
	   ...
	   for(ta=0;ta<49;ta++){
	     for(tb=0;tb<16;tb++){
	       tc=0;
	       if ((tb & 0x04) != 0){tc+=((decode_tableA2[ta]*ADPCMA_MIXING_LEVEL));}
	       if ((tb & 0x02) != 0){tc+=((decode_tableA2[ta]*ADPCMA_MIXING_LEVEL)>>1);}
	       if ((tb & 0x01) != 0){tc+=((decode_tableA2[ta]*ADPCMA_MIXING_LEVEL)>>2);}
	       tc+=((decode_tableA2[ta]*ADPCMA_MIXING_LEVEL)>>3);
	       if ((tb & 0x08) != 0){tc=(0-tc);}
	...
	void YM2612UpdateOne(int num, INT16 **buffer, int length)
	{
	    ...
	    for( i=0; i < length ; i++ )
		{
			...
			for(ch = cch[0] ; ch <= ech ; ch++)
				FM_CALC_CH( ch );
			if (dacen != 0)  *cch[5].connect4 += dacout;
			/* buffering */
			FM_BUFFERING_STEREO;
			/* timer A controll */
			INTERNAL_TIMER_A( State , cch[2] )
